...
#
# Usage: scripts/forecast-perf-gate.sh [--update]
set -uo pipefail
cd "$(dirname "${BASH_SOURCE[0]}")/.."

BIN=./target/release/brain
BASE=scripts/forecast-perf-baselines
...
gate_one() { # name target
  ...
  if [ "$update" -eq 1 ]; then
    ...
    echo "  NO BASELINE for $name (run: scripts/forecast-perf-gate.sh --update)"; fail=1
