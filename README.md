...
**393 kernels.** Every column is a field the kernel DECLARES in its own header
...
* **4/5** — workgroup-cooperative — threads share a reduction through workgroup memory (20 kernels)
* **3/5** — coalesced elementwise or gather — one thread per output, no serial reduction (184 kernels)
* **2/5** — one thread per output with a serial reduction over a Params-bounded axis (128 kernels)
* **1/5** — one thread walking a DEEP (3+ nested) serial reduction — the `conv2d_dx` shape (47 kernels)
...
op (156 kernels), not that this file runs there.
...
| [`bmm`](crates/kernels/wgsl/bmm.wgsl) | Batched matmul: out[b,m,n] = alpha * sum_k A[b,·]·B[b,·], both operands vary per batch | one thread per output element, serial inner reduction over k | 2/5 | ✓ | ✓ | — | — |
| [`bmm_acc`](crates/kernels/wgsl/bmm_acc.wgsl) | Batched matmul, accumulating: out[b,m,n] += alpha * sum_k A[b,·]·B[b,·] | one thread per output element, serial inner reduction over k | 2/5 | ✓ | ✓ | — | — |
...
| [`causal_conv1d_step`](crates/kernels/wgsl/causal_conv1d_step.wgsl) | Causal depthwise Conv1d, single-token streaming step (ring-buffer state) | one thread per (n,c) output element, serial reduction over K taps | 2/5 | ✓ | ✓ | — | — |
...
| [`decode_softmax_batched`](crates/kernels/wgsl/decode_softmax_batched.wgsl) | Batched decode softmax: per (sequence b, head h), max-subtracted softmax over its seq_lens[b] scores in a [batch, n_heads, cap]-strided buffer | one thread per (seq, head) row, 3 serial passes over seq_len | 2/5 | ✓ | ✓ | ✓ | — |
...
| [`exp`](crates/kernels/wgsl/exp.wgsl) | Elementwise exponential | one thread per output element | 3/5 | ✓ | ✓ | ✓ | — |
...
| [`flash_attn_bidir`](crates/kernels/wgsl/flash_attn_bidir.wgsl) | Flash attention (bidirectional self-attention), TILED with shared-memory K/V reuse + online softmax — Pascal-friendly (sm_61 | 64-thread workgroup tile, 2 barriers | 4/5 | ✗ | ✓ | — | — |
...
| [`gdn_add_identity`](crates/kernels/wgsl/gdn_add_identity.wgsl) | Add the identity to Gated DeltaNet's UT-transform matrix | one thread per output element | 3/5 | ✓ | ✓ | — | — |
| [`gdn_chunk_cumsum_step`](crates/kernels/wgsl/gdn_chunk_cumsum_step.wgsl) | One sequential step of Gated DeltaNet's per-chunk log-decay cumsum | one thread per output element | 3/5 | ✓ | ✓ | — | — |
| [`gdn_chunk_reverse_cumsum_step`](crates/kernels/wgsl/gdn_chunk_reverse_cumsum_step.wgsl) | One sequential step of Gated DeltaNet's per-chunk cumsum BACKWARD (suffix sum) | one thread per output element | 3/5 | ✓ | ✓ | — | — |
| [`gdn_decay_gate`](crates/kernels/wgsl/gdn_decay_gate.wgsl) | Gated DeltaNet's per-token decay gate: g = -exp(A_log) * softplus(a+dt_bias) | one thread per output element | 3/5 | ✓ | ✓ | — | — |
| [`gdn_decay_gate_bwd`](crates/kernels/wgsl/gdn_decay_gate_bwd.wgsl) | Backward of gdn_decay_gate.wgsl w.r.t. its a_proj input | one thread per output element | 3/5 | ✓ | ✓ | — | — |
| [`gdn_decay_mask`](crates/kernels/wgsl/gdn_decay_mask.wgsl) | Gated DeltaNet's per-chunk causal decay mask | one thread per output element | 3/5 | ✓ | ✓ | — | — |
| [`gdn_decay_mask_bwd`](crates/kernels/wgsl/gdn_decay_mask_bwd.wgsl) | Backward of Gated DeltaNet's causal decay mask, row-sum or column-sum by mode | one thread per output element, serial reduction over a Params-bounded axis | 2/5 | ✓ | ✓ | — | — |
| [`gdn_decay_scale`](crates/kernels/wgsl/gdn_decay_scale.wgsl) | Per-row decay scale for Gated DeltaNet's state update: exp(g_cs_last - g_cs_i) | one thread per output element | 3/5 | ✓ | ✓ | — | — |
| [`gdn_decay_scale_bwd`](crates/kernels/wgsl/gdn_decay_scale_bwd.wgsl) | Backward of Gated DeltaNet's per-row state-update decay scale, elementwise half | one thread per output element | 3/5 | ✓ | ✓ | — | — |
| [`gdn_decay_scale_bwd_last`](crates/kernels/wgsl/gdn_decay_scale_bwd_last.wgsl) | Backward of Gated DeltaNet's per-row state-update decay scale, last-row reduction half | one thread per (batch,head), serial reduction over a Params-bounded axis | 2/5 | ✓ | ✓ | — | — |
| [`gdn_layout_permute`](crates/kernels/wgsl/gdn_layout_permute.wgsl) | Token-major [B,T,H,D] <-> chunk-major [n_chunks,B,H,C,D] permute for model::gdn | one thread per output element | 3/5 | ✓ | ✓ | — | — |
| [`gdn_mask_strict_lower`](crates/kernels/wgsl/gdn_mask_strict_lower.wgsl) | Finish Gated DeltaNet's attn0: strictly-lower-triangular mask times the decay mask | one thread per output element | 3/5 | ✓ | ✓ | — | — |
| [`gdn_mask_strict_lower_bwd`](crates/kernels/wgsl/gdn_mask_strict_lower_bwd.wgsl) | Backward of Gated DeltaNet's attn0 mask-and-multiply | one thread per output element | 3/5 | ✓ | ✓ | — | — |
| [`gdn_row_scale_off`](crates/kernels/wgsl/gdn_row_scale_off.wgsl) | Per-row scalar scale with independent flat offsets into x and s (scale_row.wgsl, generalised for a chunk slice) | one thread per output element | 3/5 | ✓ | ✓ | — | — |
| [`gdn_state_decay`](crates/kernels/wgsl/gdn_state_decay.wgsl) | Scale Gated DeltaNet's whole recurrent state by one scalar per (batch,head) | one thread per output element | 3/5 | ✓ | ✓ | — | — |
| [`gdn_state_decay_bwd_dscale`](crates/kernels/wgsl/gdn_state_decay_bwd_dscale.wgsl) | Backward of Gated DeltaNet's whole-state decay scalar | one thread per (batch,head), serial reduction over a Params-bounded axis | 2/5 | ✓ | ✓ | — | — |
| [`gdn_ut_bwd_dattn0`](crates/kernels/wgsl/gdn_ut_bwd_dattn0.wgsl) | Gated DeltaNet UT-transform backward, half 1: d_attn0 from a finalised row of d_t_mat | one thread per output element, serial reduction over a Params-bounded axis | 2/5 | ✓ | ✓ | — | — |
| [`gdn_ut_bwd_dtmat`](crates/kernels/wgsl/gdn_ut_bwd_dtmat.wgsl) | Gated DeltaNet UT-transform backward, half 2: scatter row i's d_t_mat into earlier rows | one thread per output element, serial scatter over a Params-bounded axis | 2/5 | ✓ | ✓ | — | — |
| [`gdn_ut_step`](crates/kernels/wgsl/gdn_ut_step.wgsl) | One sequential row of Gated DeltaNet's intra-chunk UT-transform forward substitution | one thread per output element, serial reduction over a Params-bounded axis | 2/5 | ✓ | ✓ | — | — |
...
| [`gn_dgb2`](crates/kernels/wgsl/gn_dgb2.wgsl) | GroupNorm affine gradients, STAGE 2 of 2 — fold the partials and ACCUMULATE | one thread per output element, serial fold over P partials | 2/5 | ✓ | ✓ | ✓ | — |
| [`gn_dgb_part`](crates/kernels/wgsl/gn_dgb_part.wgsl) | GroupNorm affine gradients, STAGE 1 of 2 — partial sums for BOTH dgamma and dbeta in one pass over `dy` | one thread per partial, strided serial reduction (no barrier) | 2/5 | ✓ | ✓ | ✓ | — |
...
| [`gn_dsum2`](crates/kernels/wgsl/gn_dsum2.wgsl) | GroupNorm backward per-group reductions, STAGE 2 of 2 — fold the partials | one thread per output element, serial fold over P partials | 2/5 | ✓ | ✓ | ✓ | — |
| [`gn_dsum_part`](crates/kernels/wgsl/gn_dsum_part.wgsl) | GroupNorm backward per-group reductions, STAGE 1 of 2 — partial sums | one thread per partial, strided serial reduction (no barrier) | 2/5 | ✓ | ✓ | ✓ | — |
...
| [`gradnorm_part`](crates/kernels/wgsl/gradnorm_part.wgsl) | Per-parameter sum of squares of its gradient, as a COOPERATIVE tree reduction | 64-thread workgroup tile, 1 barrier | 4/5 | ✓ | ✓ | — | — |
...
| [`layernorm_rows`](crates/kernels/wgsl/layernorm_rows.wgsl) | LayerNorm forward, one WORKGROUP per row — the coalesced variant | 64-thread workgroup tile, 1 barrier | 4/5 | ✓ | ✓ | ✓ | — |
...
| [`matmul_q4_dyn`](crates/kernels/wgsl/matmul_q4_dyn.wgsl) | Naive W4A8 matmul (out = dequant(x_q8 @ w_q4ᵀ)) -- the correct-first, non-tiled q4 GEMM | one thread per output element, serial inner reduction | 2/5 | ✓ | ✓ | ✓ | q4 |
| [`matmul_q4_gemv`](crates/kernels/wgsl/matmul_q4_gemv.wgsl) | Skinny-M W4A8 matmul (out = dequant(x_q8 @ w_q4ᵀ)), one WORKGROUP per output COLUMN -- the decode-regime q4 GEMM | 64-thread workgroup tile, 1 barrier, serial inner reduction | 4/5 | ✓ | ✓ | ✓ | q4 |
...
| [`matmul_rows`](crates/kernels/wgsl/matmul_rows.wgsl) | Row-blocked matmul, same contract as matmul.wgsl | one thread per 8-row output block, serial contraction (register row-block, no workgroup memory) | 2/5 | ✓ | ✓ | ✓ | — |
...
| [`paged_kv_append_i8_clipped_batched`](crates/kernels/wgsl/paged_kv_append_i8_clipped_batched.wgsl) | Append a batch of new tokens' K/V into an INT8 paged pool, with a calibrated clip ceiling on the online scale | one thread per output element, serial inner reduction | 2/5 | ✓ | ✓ | — | int8 |
...
| [`prelu_bwd`](crates/kernels/wgsl/prelu_bwd.wgsl) | PReLU backward, NCHW — produces BOTH gradients in one pass | one thread per channel, 2 nested serial loops (N x H*W) — the deliberately-naive CPU fallback of prelu_bwd_wg | 2/5 | ✓ | ✓ | ✓ | — |
| [`prelu_bwd_wg`](crates/kernels/wgsl/prelu_bwd_wg.wgsl) | PReLU backward, one WORKGROUP per CHANNEL — the cooperative, COALESCED twin of prelu_bwd.wgsl | 64-thread workgroup tile, 1 barrier | 4/5 | ✓ | ✓ | ✓ | — |
...
| [`splice_add_offset_src`](crates/kernels/wgsl/splice_add_offset_src.wgsl) | Residual DeepStack add with an independent SOURCE offset: dst[dst_base+i] += src[src_base+i] | one thread per output element | 3/5 | ✓ | ✓ | — | — |
| [`splice_bwd`](crates/kernels/wgsl/splice_bwd.wgsl) | Residual splice (backward) | one thread per output element | 3/5 | ✓ | ✓ | — | — |
| [`sub`](crates/kernels/wgsl/sub.wgsl) | Elementwise subtract, with an independent flat offset into each input | one thread per output element | 3/5 | ✓ | ✓ | ✓ | — |
