...
**357 kernels.** Every column is a field the kernel DECLARES in its own header
...
* **4/5** — workgroup-cooperative — threads share a reduction through workgroup memory (24 kernels)
* **3/5** — coalesced elementwise or gather — one thread per output, no serial reduction (171 kernels)
* **2/5** — one thread per output with a serial reduction over a Params-bounded axis (103 kernels)
...
(11 kernels) — cross-checked against the barrier count on every run.
...
op (152 kernels), not that this file runs there.
...
| [`router_bwd`](crates/kernels/wgsl/router_bwd.wgsl) | Router backward: gradient w.r.t | one thread per output element, 7 nested serial reductions | 1/5 | ✓ | ✓ | — | — |
