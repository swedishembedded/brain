...
**396 kernels.** Every column is a field the kernel DECLARES in its own header
...
* **4/5** — workgroup-cooperative — threads share a reduction through workgroup memory (26 kernels)
* **3/5** — coalesced elementwise or gather — one thread per output, no serial reduction (189 kernels)
* **2/5** — one thread per output with a serial reduction over a Params-bounded axis (121 kernels)
...
op (157 kernels), not that this file runs there.
**quant** — part of the INT8 path (17 kernels) or the int4-weight/int8-activation (W4A8) q4 path (3 kernels).
