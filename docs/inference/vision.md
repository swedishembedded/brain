...
### Face detection and identity — `brain/facenet`

Finds every face in an image (boxes, scores, landmarks), and turns an
aligned face crop into an identity embedding for matching or search — "are
these two photos the same person?" It's also the identity input some
generative pipelines condition on. See
[the face recognition page](../models/face.md).

## Detection vs. editing

These four models look at an image and tell you something about it, or cut
