...
### Face detection and identity embedding - `brain/scrfd`, `brain/arcface`

`brain/scrfd` detects faces in a photo (boxes, scores, landmarks);
`brain/arcface` produces a normalized identity embedding for a face, detecting
and aligning it first unless you pass `--align false`. Use them for face
