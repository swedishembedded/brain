...
| [`attn_relpos_add`](../../crates/kernels/wgsl/attn_relpos_add.wgsl) | Decomposed relative-position bias, step 2 - fold the two q·R intermediates into the score slab | one thread per output element | 3/5 | ✓ | ✓ | - | - |
| [`attn_relpos_dq`](../../crates/kernels/wgsl/attn_relpos_dq.wgsl) | Decomposed relative-position bias backward - the d(q) contribution, ACCUMULATED into the fused d_qkv | 8-row register block per thread, coalesced on head_dim, serial reduction over k_ext | 2/5 | ✓ | ✓ | - | - |
| [`attn_relpos_dr`](../../crates/kernels/wgsl/attn_relpos_dr.wgsl) | Decomposed relative-position bias backward - the dense-table adjoint, accumulated over heads/rows/spans | 8-column register block per thread, coalesced on head_dim, 2 nested serial reductions | 2/5 | ✓ | ✓ | - | - |
| [`attn_relpos_drh`](../../crates/kernels/wgsl/attn_relpos_drh.wgsl) | Decomposed relative-position bias backward - d(rel_h), the CONTIGUOUS segment sums of d_scores | 64-thread workgroup tile, 1 barrier | 4/5 | ✓ | ✓ | - | - |
| [`attn_relpos_drw`](../../crates/kernels/wgsl/attn_relpos_drw.wgsl) | Decomposed relative-position bias backward - d(rel_w), the STRIDED column sums of d_scores | one thread per output element, serial inner reduction | 2/5 | ✓ | ✓ | - | - |
| [`attn_relpos_qr`](../../crates/kernels/wgsl/attn_relpos_qr.wgsl) | Decomposed relative-position bias, step 1 - the q·R hoist (SAM/ViTDet `add_decomposed_rel_pos`) | 8-row register block per thread, coalesced on the table axis, serial reduction over head_dim | 2/5 | ✓ | ✓ | - | - |
...
| [`router_gate`](../../crates/kernels/wgsl/router_gate.wgsl) | Router gating: softmax over experts -> keep top_k -> optional renorm + scale | one thread per token, array-free (no expert-count cap) | 1/5 | ✓ | ✓ | - | - |
...
| [`router_gate_train`](../../crates/kernels/wgsl/router_gate_train.wgsl) | Router gating (training variant): softmax + full probs + top_k gate (optional renorm + scale) | one thread per token, array-free (no expert-count cap) | 1/5 | ✓ | ✓ | - | - |
