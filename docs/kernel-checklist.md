...
| registering `matmul_gemv` in `crates/unet` is "small but free" — SDXL's `m = 1` conditioning chain runs the naive kernel for ~6 ms | **KILLED — 2–4x on the kernel, ZERO on the pass.** `gemv` genuinely wins every `m = 1` shape ([1,1280,1280] 0.10 vs naive 0.21 vs reg3 0.45 ms; [1,2816,1280] 0.19/0.43/0.94) and the measured crossover is `m ≤ 8` across three shapes — but the whole forward is **1254.53 → 1254.84** and **1256.60 → 1253.97 ms**, i.e. nothing, because the grouped 3.6 ms was drain (`docs/lessons.md` #21). 21 of 2196 dispatches cannot move a 1250 ms pass whatever they cost. Reverted: it is not free, it changes the conditioning path's output (max delta **2.5e-3** vs the naive reference — the cooperative fold reassociates the sum) and adds a `workgroupBarrier` kernel to a model that must also run on `backend-cpu`, all for no wall clock. Same shape as the `bias_grad` revert. **Before optimizing a kernel, check what fraction of the PASS it can possibly return** |
| composing LayerNorm2d from `nchw_nlc` → `layernorm_rows` → `nlc_nchw` beats a fused kernel, because the `*_rows` stage is coalesced | **KILLED — fusing wins at every shape.** The two permutes were 67–86% of it, and both pay the sector amplification that was the stated reason to avoid fusing, so the composition paid it TWICE to avoid paying it once. `layernorm2d` (one invocation per spatial position, walking `C` at stride `H*W`) measures **4.38x at [1,32,65536]**, 2.72x at [1,64,16384], 1.34x at [1,256,4096], 1.09x at [1,96,1024] — the margin growing with `H*W` exactly as the earlier bandwidth numbers predicted. Gated against a host oracle in `crates/gradcheck/tests/layernorm2d_kernels.rs` (max delta 7.2e-7). **Not yet wired**: `vision::blocks::LayerNorm2d`'s forward populates the NLC `xt`/`yt` buffers its backward reads, so adopting the fused kernel needs either an eval-only branch (the shape `vae::blocks::conv_s` uses for its own lowering) or a fused backward as well |
