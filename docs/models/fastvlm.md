...
Model id: `brain/fastvlm` (reserved vendor - never auto-fetched).
`BRAIN_FASTVLM_WEIGHTS` - a checkpoint directory holding `config.json` +
`model.safetensors` + `tokenizer.json` (overridable per call via the
`weights` param).
