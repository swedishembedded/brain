...
No dedicated `brain sam2` verb - use the generic capability action
`segment`, over the CLI or D-Bus. It takes a required `image` input plus
...
through the CLI's generic capability dispatch or D-Bus.
