...
Atari-100k-style world model) is the one you can play end to end today.
Reach for it to interactively probe what a world model has actually learned,
to record and replay episodes for offline evaluation, or to fine-tune it on
your own recorded episodes.
...
brain wm bench --model fake|diamond [--weights F] [--onnx F.onnx] [--frames N] [--seed N]
...
- `finetune`: `--steps` (default 300), `--lr` (default 1e-4), `--wd`
  ...
  Trains an episode dataset produced by `brain data gen` or `brain wm play
  --record`; refuses to save a run that diverged (NaN/inf loss).

## Hardware and limits

- CLI only (`brain wm ...`) — no D-Bus and no HTTP route.
