...
### 6.2 What the paper reports, and what reproduces

SalesRLAgent's Table I reports **0.967 accuracy / 0.98 AUC-ROC**, against 0.69
for XGBoost and 0.75 for a hybrid LLM+ML system. This sample is far below that,
and the gap is worth taking apart rather than apologizing for, because most of
it is not a modelling gap.

Measured here on the published 100k dataset:

| | accuracy | AUC-ROC |
|---|---:|---:|
| constant predictor | 0.500 | 0.500 |
| **salesagent** (this sample) | 0.707 | 0.798 |
| bag-of-words logistic | 0.730 | 0.805 |
| **Azure 3072-d embeddings + logistic** | **0.798** | **0.878** |
| threshold `sales_effectiveness` | 0.922 | - |
| threshold `customer_engagement` | 0.927 | - |
| **threshold `probability_trajectory`** | **0.982** | - |
| *SalesRLAgent, as reported* | *0.967* | *0.98* |

Two things follow.

**The reported number is reachable without reading the conversation.** The
dataset ships `probability_trajectory`, `customer_engagement` and
`sales_effectiveness`, all generated by GPT-4o alongside the outcome.
Thresholding the trajectory's final value alone gives **0.982** - above the
paper's headline. Any pipeline that admits those columns as *features* is
predicting the outcome from a paraphrase of the outcome. This sample uses the
trajectory strictly as a training TARGET and never as an input, which is why
its numbers look nothing like 0.967.

**The paper's own input representation does not reach it either.** Logistic
regression on the dataset's Azure OpenAI embeddings - the exact representation
the paper depends on - gets **0.798 / 0.878** (3000 train, 1000 held out).
That is 17 points short of the reported accuracy, and it is the realistic
ceiling for predicting conversion from conversation text on this data.

There is also an ablation row in the paper that cannot be a measurement:
"- Caching optimization: 0.95 (-0.017)". Caching is a latency optimization and
cannot change what a model predicts.

So the honest reading of this sample's 0.707 is **nine points below a ~0.80
ceiling**, not twenty-six points below a real 0.967. Notably, the paper's one
physically-meaningful ablation row - "- Azure OpenAI embeddings: -0.077" - is
almost exactly the gap between this sample and that ceiling, and swapping a
3072-d cloud embedding model for a 384-d local one is precisely the deviation
[section 7](#7-faithful-adapted-not-done) names.
