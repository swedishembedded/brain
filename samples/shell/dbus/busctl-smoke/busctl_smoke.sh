...
#
#     tools/dbus-session.sh -- bash samples/shell/dbus/busctl-smoke/busctl_smoke.sh [path/to/brain]
#
...
if [[ -z "${DBUS_SESSION_BUS_ADDRESS:-}" ]]; then
  echo "no session bus - run me under:  tools/dbus-session.sh -- bash $0" >&2
  exit 1
fi
