...
later `w` costs one forward.
    ...
    BRAIN_CODEFORMER_WEIGHTS=/path/to/codeformer \\
      tools/dbus-session.sh --serve "--dbus" -- \
        python3 samples/python/restore/restore-face/restore_face.py --image face.ppm
...
(`samples/python/vision/face-id/face_id.py`) to find the face in a full photo first.
"""
...
def main() -> int:
    ...
    with BrainDBus() as brain:
        if "brain/codeformer" not in brain.models():
        ...
        for value in [float(v) for v in args.w.split(",")]:
            t = time.perf_counter()
            r = brain.run(
                "brain/codeformer", "restore_face", {"w": value},
