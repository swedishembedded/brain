...
| `tests/e2e/samples.bats` + `tests/e2e/samples_manifest.bats` | `test/e2e/samples` | Every sample script under `samples/{shell,python}/` actually runs (against the mock, where the model it needs has a mock equivalent) or skips with a clear, honest reason - not silently. The completeness check (its own bats file, not gated behind the harness's own skip conditions) fails the suite if a tracked sample script is missing from `tests/e2e/samples/manifest.tsv`, so a newly-added, never-wired sample cannot rot the way examples used to before this suite existed. |
