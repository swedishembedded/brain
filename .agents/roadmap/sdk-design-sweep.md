...
- [ ] **Phase 2** - new pipelines, in the priority order above: Forecast, Text, Embedding, ASR, then the rest. Each gets its own sub-roadmap section here (or its own file, linked from here) when it starts, written against the full `sdk-design.md` checklist from day one - including an end-to-end test, learning from M10 rather than repeating the `flux2_cli.rs` duplication gap a second time. Design each pipeline's progress/cancellation surface (rule 8) toward the `run.start()/subscribe()/cancel()/result()` shape `.agents/roadmap/orchestration-hsm.md` proposes, rather than reinventing M5's synchronous `generate_with_progress(cancel, on_progress)` a second time - M5's shape stays the right SIMPLE default, but a new pipeline's ADVANCED tier should point at where this is heading.
  ...
  - [x] **Phase 6.4** - `download_policy` on every OTHER single-architecture pipeline builder: depth, embedding, restore, ground, music, video, vlm, text, detect, segment, upscale, asr (12 builders). Factored the resolve-then-fetch-on-`Missing` sequence Phase 3.4/4.2/5.1b/6.1/6.2 each independently carried into one shared `crate::resolve_policy::resolve_with_policy(arch, spec, model_id, overrides, download_policy)`, rather than folding the same three-way policy match into 12 separate copies - real technical debt those nine near-identical blocks were already accumulating, now one implementation instead of one-per-file drift risk. `detect`/`segment`/`upscale`/`asr` (yolov8/sam2/rrdbnet/qwen3asr, all recipe-backed, "safe by design" per Phase 3.4's own audit) were ALSO switched from their original naive `Store::local`-then-resolve order onto the same resolve-first helper - closes no live bug for these four (their recipes already made the naive order safe), but removes the now-pointless two-strategy split and gives them the SAME `Offline`/`AlwaysCheck` knob for free; verified safe by running each one's real end-to-end forward-pass test afterward (`detection_pipeline`/`segment_pipeline`/`upscale_pipeline`/`transcribe_pipeline`, all still green, including `upscale_pipeline`'s own two-candidate `Ambiguous` test). Every one of the 12 touched test files gained a `download_policy_offline_never_touches_the_network` test, the same loopback-port proof Phase 6.3 introduced. `text::resolve_hub_weights` (the hub-id half of `TextGenerationPipelineBuilder::load`, which also accepts a literal local path) is the thirteenth caller - same helper, different call site shape. Deliberately NOT touched: `TtsPipeline`/`ForecastPipeline`, each a two-architecture dispatch (qwen3tts+cosyvoice, kronos+timesfm3) needing their own tie-break-aware policy match - `resolve_policy.rs`'s own module doc names this explicitly as a second generalization not needed yet, the same reasoning `ImagePipelineBuilder`'s own two-arch dispatch was left with its bespoke inline version in Phase 6.3.
  - [x] **Phase 6.6** - `AutoPipeline`: closes `sdk.md`'s tracked gap. Hand `AutoPipeline::from_pretrained` ANY `<vendor>/<repo>` this crate has a pipeline for and get the right concrete pipeline TYPE back - an enum, one boxed variant per pipeline (15 total: `Image`, `Upscale`, `Restore`, `Forecast`, `Embedding`, `Detection`, `Segment`, `Depth`, `Grounding`, `Transcribe`, `Tts`, `Music`, `Video`, `VisionLanguage`, `Text`), never a trait object - every pipeline's own call shape genuinely differs (`Image` -> `Image`, `Embedding` -> `Embedding`, `Detection` -> `Vec<Detection>`, ...), so there is no common trait to erase behind without either losing real capability or forcing every method onto every pipeline whether it fits or not, the same "one type per capability" reasoning `ImagePipeline` itself already applies fronting flux2 AND s3dit. Boxed uniformly (not just the outlier) after `clippy::large_enum_variant` flagged `yolov8::Yolo`'s own >16KB inline footprint - re-flagged the SECOND-largest variant once the first was boxed alone, so one consistent rule replaced chasing the threshold variant by variant. Dispatch tries every known architecture's `loader::resolve_structured` against the CURRENT store, first match wins (the same "ambiguous beats missing, first Missing is the arbitrary tie-break" preference `crate::resolve_policy::try_two` already applies pairwise, generalized here to fifteen), then builds through THAT pipeline's own real public `builder(model_id)...load()` - so the code that actually decides what gets built is the exact same code `DepthPipeline::from_pretrained` (or any other concrete constructor) already runs, resolved a second time rather than duplicated; a real, accepted trade-off, the same one `crate::resolve_policy::resolve_with_policy`'s own `Missing`-then-retry already makes for a one-shot construction call. New `auto` Cargo feature (`image`+`forecast`+`vision`+`audio`+`video`+`multimodal` as a hard bundle, not an optional subset - the whole promise is "any supported model id") needed a genuine, documented exception in `scripts/gates/check-sdk-features.sh`'s own rule-1 check: `auto` is a DISPATCH surface crossing every `Domain`, not a claim about one architecture's modality, so it earns a named `CROSS_CUTTING` allowlist entry rather than bending that check for everyone. Verified with two REAL, DIFFERENT fixtures in the same test file (a ZipDepth checkpoint dispatching to `AutoPipeline::Depth` and reaching a real `.predict()`; a YOLOv8 checkpoint dispatching to `AutoPipeline::Detection` and reaching a real `.detect()`) - the actual "auto" claim, not just that resolution succeeds once.
  - [x] **Phase 6.5** - `download_policy` on the crate's last two holdouts: `TtsPipeline` (qwen3tts/cosyvoice) and `ForecastPipeline` (kronos/timesfm3), the two remaining pipelines that dispatch across TWO architectures with a tie-break, which Phase 6.4 explicitly deferred. Added `crate::resolve_policy::resolve_two_with_policy` (plus its own `try_two` tie-break helper and `Resolved2` carrier enum) alongside the single-arch `resolve_with_policy` - the natural generalization now that THREE real call sites wanted the same shape (`tts`, `forecast`, and `pipeline.rs`'s own inline version, which stays inline anyway since it alone also reports download/build progress through caller-supplied closures - folding progress support into the shared helper would mean adding an unused parameter to every other caller). `forecast.rs` also moved off its original naive `Store::local`-then-resolve order onto the shared resolve-first helper, the same class of "closes no live bug, only unifies the strategy" change Phase 6.4 made for detect/segment/upscale/asr - kronos/timesfm3 both have working recipes. `tts.rs` was ALREADY resolve-first (Phase 4.2 fixed that the hard way, for cosyvoice specifically), so this only adds the policy knob there, changing no ordering. Verified via both pipelines' full existing test suites (qwen3tts AND cosyvoice AND cosyvoice3 resolution, kronos AND timesfm3 resolution - every tie-break branch re-exercised, not just one side), plus a new `download_policy_offline_never_touches_the_network` test on each. This closes `sdk.md`'s `DownloadPolicy` gap completely - every pipeline builder in the crate now has the knob.
  - [x] **Phase 6.3** - `ImagePipelineBuilder::download_policy`, closing `sdk.md`'s own "Not yet done" `DownloadPolicy` gap for the `image` surface: a new builder knob (default `loader::DownloadPolicy::IfMissing`, unchanged behavior) whose `Offline` arm never calls `brain_modelstore::plan`/`execute_plan` at all - proven by `tests/image_pipeline.rs`'s `download_policy_offline_never_touches_the_network`, which points `HfHub` at a loopback port nothing listens on (`BRAIN_HUB_ENDPOINT`) and shows a reference resolving neither locally nor from any real hub still comes back a clean `Error::Missing`, never a connection-error-flavored `Error::Download` - the same class of "prove the negative, don't just reason about it" test this sweep has used throughout. `AlwaysCheck` reuses `loader::supply::ensure_default_weights_with`'s own already-established semantics (always run plan/execute, never skip on a local hit) rather than inventing a third reading of the type. Scoped to `ImagePipeline` only - the pipeline `sdk.md`'s gap names specifically; the same knob on the other resolve-based pipeline builders is a mechanical follow-up, not done here.
