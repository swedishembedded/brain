...
| `promote` | the **model-agnostic promote/reject decision**: the `Environment`/`Verifier` reward seam, the exact one-sided paired sign test, the four-bar gate over already-scored pairs, and the document/fact probe contract (`FactBatch`/`DocumentVerifier`/`document_gate_config`). A **leaf** - it depends on no other brain crate, on purpose: a layer-4 model crate has to be able to gate its own candidate adapter, and with this machinery inside `rl` that was the Cargo cycle `brain-qwen3 -> brain-rl -> brain-bench -> brain-qwen3`. `rl::{env,gate}` and `bench::metrics::sign_test` are re-exports of it, so every existing caller is unchanged and no logic exists twice. `brain::promote` (SDK, under the `decision` surface) is how a SAMPLE reaches it - `samples/decision/doom`'s `gate` command decides whether a newly trained policy may replace the one in service |
| `rlcd` | **RLCD (reinforcement learning for calibrated decisions)**: proper scoring rules over soft target distributions (`scoring`), cost-sensitive decision theory - `CostMatrix`, Bayes action/risk, regret, value of information (`cost`) - calibration metrics - ECE/AdaECE/classwise-ECE/NLL/Brier/reliability/coverage/failure-AUROC (`metrics`) - the `World`/`DecisionContract` seam an executable probabilistic world implements plus the `prior = E[posterior \| evidence]` oracle-honesty identity (`atlas`), and exhaustive learner-error witness search with a corrective family (`witness`). A **leaf** for the same reason as `promote`: this used to live inside `brain-decide` (layer 4), unreachable by any OTHER decision-capable model crate. `decide::loss` re-exports `scoring` unchanged. `brain::RlcdPipeline` (SDK) and `samples/learning/rlcd` are the served/demonstrated surface - see `.agents/roadmap/rlcd.md` |
| `search` | the **discovery runtime**: a quality-diversity archive keyed by a behavioural `Niche` with one elite per niche and cost as the tiebreaker (`archive`), a UCB1 allocator over search operators scored on measured archive gain PER SECOND (`allocate`), and an evaluator cascade that refuses cheaply before it verifies expensively (`cascade`). A **leaf** like `promote` and `rlcd`, and for a stronger reason: it knows nothing of models, devices, environments or games, so the same archive hosts MCTS, evolution, Bayesian optimisation or an LLM proposal operator behind one interface. `brain::search` (SDK) re-exports it unchanged; `samples/decision/doom` is the worked case - see `.agents/roadmap/doom.md` |
