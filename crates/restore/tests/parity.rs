...
use restore::{CodeFormer, CodeFormerConfig};
...
fn gpu() -> gpu_core::Gpu {
    match std::env::var("BRAIN_RESTORE_DEVICE").as_deref() {
        Ok("cpu") => gpu_core::Gpu::new_cpu(&restore::KERNELS),
        _ => gpu_core::testgpu::dev(&restore::KERNELS),
    }
}
...
fn imported(cfg: &CodeFormerConfig) -> Option<vae::blocks::Tensors> {
    let path = weights()?;
    let im = restore::import::load(&path, cfg).unwrap_or_else(|e| panic!("import {path}: {e}"));
    println!(
