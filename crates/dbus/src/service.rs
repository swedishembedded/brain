...
impl Manager {
    ...
    /// reflected to the caller (an error-hygiene requirement).
...
impl Manager {
    ...
    /// (`StatsSnapshot.models` filtered to `resident == true`, each with its
    /// per-device instances) - a client wanting to show what is warm no
    /// longer has to pull and parse the whole `StatsSnapshot` tree just to
    /// answer that. Read-only, no arguments: same data `StatsSnapshot`
...
/// testable without a live zbus interface object, the same way
/// `brain_stats::snapshot_from_executor` itself is tested.
fn resident_models_json(exec: &Executor) -> String {
    let resident: Vec<brain_stats::ModelStat> =
        brain_stats::snapshot_from_executor(exec).models.into_iter().filter(|m| m.resident).collect();
    serde_json::to_string(&resident).unwrap_or_else(|_| "[]".into())
}
...
mod tests {
    ...
    use super::{resident_models_json, resolve_model_alias, Manager};
    ...
    /// SPEC: `ResidentModels` is a convenience projection of `StatsSnapshot`,
    /// not a second source of truth - a client should never see it disagree
    /// with `stats_snapshot().models` filtered to `resident == true`. Exercised
    /// against a real (if minimal) executor rather than hand-built JSON, so a
    /// future change to either method's plumbing cannot let them drift apart
    /// silently.
    #[test]
    fn resident_models_matches_the_resident_subset_of_stats_snapshot() {
        let mgr = Manager::new(empty_exec());
        mgr.executor.register(std::sync::Arc::new(StubResident("brain/asr-stub".to_string())));

        let snap = brain_stats::snapshot_from_executor(&mgr.executor);
        let want: Vec<brain_stats::ModelStat> = snap.models.into_iter().filter(|m| m.resident).collect();

        let got: Vec<brain_stats::ModelStat> =
            serde_json::from_str(&resident_models_json(&mgr.executor)).expect("valid resident_models JSON");
        assert_eq!(got.len(), want.len());
        for m in &got {
            assert!(m.resident, "resident_models must never list a non-resident model: {}", m.id);
        }
        assert_eq!(got.iter().map(|m| &m.id).collect::<Vec<_>>(), want.iter().map(|m| &m.id).collect::<Vec<_>>());
    }
