...
//! no bespoke harness, because [`deepseek2::DeepseekV2`] implements
...
use deepseek2::model::{DeepseekV2, PIPELINES};
...
/// of [`deepseek2::init::init_weights`]'s production `0.02`.** Both numbers
...
fn grads_match_finite_differences_lora() {
    ...
    let cfg = DeepseekV2Config { lora: Some(deepseek2::config::lora_cfg(2, 4.0)), ..cfg_of(4, 4, false, 1.0) };
    let m = harness(7, cfg);
