...
//! --lora` uses (`qwen::finetune::finetune`), differing ONLY in which
//! completion their training data supervises for one fixed prompt --
//! reloads both from disk (not the live in-process model -- see
//! docs/lessons.md on the config field that used to silently drop on
...
//! possible optimization problem (`crates/qwen/tests/lora_roundtrip.rs`
...
fn train_adapter(base_path: &str, target: u32, out_dir: &Path, adapter_out: &Path) -> (f32, f32) {
    ...
    // (docs/lessons.md §4: a degenerate rank equal to head_dim or d_model
    // would hide a whole shape-transposition bug class) -- same choice as
    // crates/qwen/tests/lora_roundtrip.rs.
    qwen::finetune::finetune(base_path, out_dir, &opts, &qwen::finetune::Mode::Lora { rank: 3, alpha: 6.0 }, adapter_out.to_str().unwrap())
        .expect("finetune (the same qwen::finetune::finetune brain qwen finetune --lora calls)")
}
...
fn lora_adapters_trained_on_different_targets_diverge_and_match_their_own_data_after_a_reload() {
    ...
    let base_init = qwen::init_weights(&base_cfg, 7);
    ...
    // the thing this gate hangs its verdict on (docs/lessons.md §16: a
