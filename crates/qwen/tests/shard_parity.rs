...
//! Pipeline-parallel sharding is bit-exact against the single-device model.
//!
...
//! optimiser across stages.
//!
//! Runs two stages on GPUs 0 and 1 by default; set `SHARD_TEST_GPUS=1,1` to put
//! both on one card (e.g. while the other is busy) — still exercises the full
//! cross-stage transfer path. Skipped under `MOE_SKIP_GPU_TESTS`.

use qwen::{Pipeline, Qwen, QwenConfig};
...
fn shard_forward_and_grad_parity() {
    ...
    // Two-stage pipeline from the SAME weights.
    let pipe = Pipeline::new(cfg.clone(), b, t, &init, true, &stage_gpus());
    assert_eq!(pipe.n_stages(), 2);
    pipe.zero_grads();
    let l_pipe = pipe.forward(&x, &y);
    pipe.backward();
    pipe.poll_wait();
...
fn shard_overfit_reduces_loss() {
    ...
    let pipe = Pipeline::new(cfg, b, t, &init, true, &stage_gpus());
    let before = pipe.forward(&x, &y);
    for step in 1..=50 {
        pipe.zero_grads();
        pipe.forward(&x, &y);
        pipe.backward();
