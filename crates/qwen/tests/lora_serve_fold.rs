...
//! (`qwen::lora::fold_adapter_into`, already proven exact against the live
//! unfolded forward by `crates/qwen/tests/lora_adapter_file.rs`), then build
...
fn decode_only_model_from_folded_tensors_matches_the_live_trained_forward() {
    ...
    let base_init = qwen::init_weights(&base_cfg, 21);
    ...
    let mut init: HashMap<String, Vec<f32>> = qwen::init_weights(&lora_cfg, 21);
    ...
    qwen::lora::save_adapter(adapter_path.to_str().unwrap(), &trained, "test/adapter", "test/base", None).unwrap();
    ...
    qwen::lora::fold_adapter_into(&mut base_tensors, adapter_path.to_str().unwrap()).expect("fold_adapter_into");

    let prompt = &x[..6];
    let live_greedy = qwen::sample::generate(&trained, prompt, 6, 0.0, 0, 1.0, None, &mut Rng::new(1));
    ...
    let served_greedy = qwen::sample::generate_kv(&served, prompt, 6, 0.0, 0, 1.0, None, &mut Rng::new(1));
