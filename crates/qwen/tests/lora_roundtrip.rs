...
//! and `lora_fwd` never dispatched. See docs/lessons.md.
...
/// rank=3 is coprime with tiny()'s head_dim=8 and d_model=16 (docs/lessons.md
...
fn lora_adapter_survives_save_and_reload() {
    ...
    let base_init = qwen::init_weights(&base_cfg, 11);
    ...
    // base weights used above -- exactly what `qwen::finetune::finetune` does
    // when seeding from a checkpoint.
    let mut init: HashMap<String, Vec<f32>> = qwen::init_weights(&lora_cfg, 11);
...
fn checkpoint_without_lora_key_loads_as_plain_model() {
    ...
    let init = qwen::init_weights(&cfg, 3);
