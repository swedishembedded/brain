...
/// `qwen3omnimoe::import::map_thinker` writes (`thinker.blocks.{l}.mlp.experts.{e}.
...
impl ThinkerInt8Store {
    ...
    /// that was not actually quantized the way `qwen3omnimoe::import` promises fails
...
/// Talker layer `l` - `qwen3omnimoe::import::map_talker`'s convention
    ...
    for l in layers {
        for e in 0..cfg.n_experts as usize {
            for leaf in ["gate", "up", "down"] {
                ...
                let (n, kg) = (shape[0], shape[1]); // shape is [n, k/4] (packed) per qwen3omnimoe::import's own plan
...
mod tests {
    ...
    fn store_weights_produce_correct_expert_fwd_i8_output() {
        ...
        g.submit(&[], &model::int8::quant_rows_steps(&g, model::int8::QuantRows { kernels: ids8.quant, x: &x, sx: &sx, xq: &xq, xgs: None }, 0, m, d));
