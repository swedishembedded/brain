...
//! `qwen3omnimoe::thinker::decode` -- tiny-config smoke test proving the full-stack
//! composition (N `layer_fwd` calls chained residual-to-residual, then the
//! top-level final RMSNorm `layer_fwd` itself deliberately omits) is wired
//! correctly, ahead of any real-weight run. Per the porting playbook, this is
//! the tier that comes BEFORE real weights: no HF checkpoint involved, no
//! `#[ignore]`, synthetic random weights via `data::rng::Lcg` (never `rand`,
//! per this engine's test convention).
//!
//! `decode`'s own correctness beyond "compose `layer_fwd` N times + norm" is
//! already covered by `thinker_layer_parity.rs`'s real-weight, single-layer
//! parity test (which now also exercises the real `qwen3vl::mrope` table this
//! test builds the same way) -- this test's only job is the composition seam
//! itself: does `decode` produce exactly what calling `layer_fwd` by hand,
//! layer by layer, followed by one more `rmsnorm_fwd`, would produce.

use data::rng::Lcg;
use model::block::{rmsnorm_fwd, KernelIds};
use qwen3omnimoe::config::MoeTextConfig;
use qwen3omnimoe::thinker::{decode, layer_fwd, thinker_pipelines, ThinkerLayerWeights, ThinkerWeights};
...
fn tiny_config(n_layers: u32) -> MoeTextConfig {
    MoeTextConfig {
        ...
        n_heads: 2,
        n_kv_heads: 1,
        head_dim: 8, // decoupled from hidden, matching the real Thinker's own decoupling
        moe_intermediate: 6,
...
fn weights<'a>(b: &'a LayerBufs) -> ThinkerLayerWeights<'a> {
    ThinkerLayerWeights { ln1: &b.ln1, wq: &b.wq, wk: &b.wk, wv: &b.wv, wo: &b.wo, q_norm: &b.q_norm, k_norm: &b.k_norm, ln2: &b.ln2, router: &b.router, experts: &b.experts }
}
...
fn rmsnorm_rows_slot() -> usize {
    thinker_pipelines().iter().position(|(n, _)| *n == "rmsnorm_rows").expect("the crate no longer registers rmsnorm_rows")
}
...
fn decode_matches_hand_chained_layer_fwd_plus_final_norm() {
    ...
    // Diagonal M-RoPE table -- same construction a pure-text real prompt uses.
    ...
    let layers: Vec<ThinkerLayerWeights> = layer_bufs.iter().map(weights).collect();
    let final_norm = gpu.storage_init("final_norm", &rng.vec_scaled(cfg.hidden as usize, 0.3));

    // The composition under test.
    let w = ThinkerWeights { layers: &layers, final_norm: &final_norm };
    ...
    // Hand-chained oracle: identical layer_fwd calls, in the same order, plus
    // one more rmsnorm_fwd -- decode should be exactly this, not a re-derivation.
    let mut h = x;
    for lw in &layers {
