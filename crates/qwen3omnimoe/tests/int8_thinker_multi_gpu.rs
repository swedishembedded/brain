...
//! `qwen3omnimoe::int8_thinker_resident::Int8ThinkerResident` on REAL, physically
...
use data::rng::Lcg;
use qwen3omnimoe::int8_resident::ThinkerInt8Store;
use qwen3omnimoe::int8_thinker_resident::{load_layer_bufs, weights, Int8ThinkerResident};
use paramstore::upload::Uploader;
use qwen3omnimoe::thinker::{layer_fwd, thinker_pipelines};
...
fn sharded_two_gpu_forward_matches_unsharded_single_gpu_forward() {
    ...
    let resident = Arc::new(Int8ThinkerResident::new(path.to_str().unwrap().to_string(), qwen3omnimoe::config::ThinkerConfig::defaults().with_text(cfg.clone()), caps));
    mgr.register_multi(resident);

    let (claimed, devices, key) = mgr.claim_multi(qwen3omnimoe::int8_thinker_resident::MODEL, "forward", &capability::Invocation::new(), &Default::default()).expect("claim_multi");
...
fn sharded_generate_matches_unsharded_generate() {
    ...
    let sharded_resident = Int8ThinkerResident::new(path.to_str().unwrap().to_string(), qwen3omnimoe::config::ThinkerConfig::defaults().with_text(cfg.clone()), caps_for_split(path.to_str().unwrap(), &cfg, &[Device::Gpu(0), Device::Gpu(1)], 2));
    let whole_resident = Int8ThinkerResident::new(path.to_str().unwrap().to_string(), qwen3omnimoe::config::ThinkerConfig::defaults().with_text(cfg.clone()), caps_for_split(path.to_str().unwrap(), &cfg, &[Device::Gpu(0)], 1));
...
fn generate_first_token_matches_independently_assembled_reference() {
    ...
    let resident = Int8ThinkerResident::new(path.to_str().unwrap().to_string(), qwen3omnimoe::config::ThinkerConfig::defaults().with_text(cfg.clone()), caps1.clone());
    let key = InstanceKey::new(qwen3omnimoe::int8_thinker_resident::MODEL, "default");
    ...
    let embed_table = qwen3omnimoe::int8_thinker_resident::load_mat_host(&reader, "thinker.embed_tokens.weight", cfg.vocab, cfg.hidden);
    let mut x_host = Vec::with_capacity(prompt_ids.len() * d);
    for &t in &prompt_ids {
        x_host.extend_from_slice(&embed_table[t as usize * d..(t as usize + 1) * d]);
    }

    let resident2 = Int8ThinkerResident::new(path.to_str().unwrap().to_string(), qwen3omnimoe::config::ThinkerConfig::defaults().with_text(cfg.clone()), caps1);
    ...
    let gpu = gpu_core::testgpu::dev(qwen3omnimoe::thinker::thinker_pipelines());
    let mut up = Uploader::new(&gpu);
    let norm_w = qwen3omnimoe::int8_thinker_resident::load_vec(&mut up, &reader, "thinker.norm.weight");
    let lm_head_w = qwen3omnimoe::int8_thinker_resident::load_mat(&mut up, &reader, "thinker.lm_head.weight");
    let h1 = gpu.storage_init("h1", last_row);
    let normed = qwen3omnimoe::thinker::final_norm(&gpu, &cfg, &norm_w, &h1, 1);
    let logits = qwen3omnimoe::thinker::lm_head_fwd(&gpu, &lm_head_w, &normed, 1, cfg.hidden, cfg.vocab);
