...
//! synthetic on-disk checkpoint with real `qwen3omnimoe::import` naming/dtypes for
...
pub fn caps_for_split(path: &str, cfg: &MoeTextConfig, devices: &[residency::Device], n: usize) -> Vec<(residency::Device, u64)> {
    ...
    let full_cfg = qwen3omnimoe::config::ThinkerConfig::defaults().with_text(cfg.clone());
    let probe = qwen3omnimoe::int8_thinker_resident::Int8ThinkerResident::new(path.to_string(), full_cfg.clone(), Vec::new());
    let total = probe.total_device_bytes().expect("synthetic checkpoint must be measurable");
    let key = residency::InstanceKey::new(qwen3omnimoe::int8_thinker_resident::MODEL, "default");
    const STEPS: u64 = 128;
    for i in 1..=STEPS {
        let cap = (total * i).div_ceil(STEPS);
        let caps: Vec<(residency::Device, u64)> = devices.iter().map(|&d| (d, cap)).collect();
        let r = qwen3omnimoe::int8_thinker_resident::Int8ThinkerResident::new(path.to_string(), full_cfg.clone(), caps.clone());
...
/// A tiny synthetic int8 checkpoint, real `qwen3omnimoe::import` naming/dtypes for
