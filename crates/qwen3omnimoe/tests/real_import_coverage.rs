...
//! `qwen3omnimoe::import::hf_to_brain` against EVERY one of the real checkpoint's
...
fn every_real_tensor_name_maps_or_is_a_known_qkv_leaf() {
    ...
    for name in weight_map.keys() {
        ...
        match qwen3omnimoe::import::hf_to_brain(name) {
            Some(bn) => {
    ...
    println!("qwen3omnimoe::import::hf_to_brain covers all {} non-qkv-leaf real tensor names, no collisions.", weight_map.len());
}
...
fn full_real_import_produces_a_two_way_covered_checkpoint() {
    ...
    qwen3omnimoe::import::import_as(dir.to_str().unwrap(), &out_path, Some("omni-full-import-test")).expect("import_as must succeed against the real, complete checkpoint");
    ...
    for name in weight_map.keys() {
        ...
        if let Some(bn) = qwen3omnimoe::import::hf_to_brain(name) {
            expected.insert(bn);
        }
    }
    for b in qkv_layers {
