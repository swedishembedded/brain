...
//!    `deepseekocr::prompt` a checked fact rather than a failed search.
...
use deepseekocr::config::DeepseekOcrConfig;
use deepseekocr::prompt::{self, build_prompt, ImageTokens};
...
fn real_tokenizer_reserved_tokens_and_prompt() {
    ...
    let plan = deepseekocr::row_plan(g, deepseekocr::ViewGrid::global_only());
