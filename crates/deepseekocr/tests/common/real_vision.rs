...
//! `deepseekocr::import`, production code, the moment the served path needed the
...
pub use deepseekocr::import::{EXPANDED, LM, MMPROJ, STORE};
...
/// The encoder's whole init map - `deepseekocr::import::encoder_weights`, the
...
pub fn encoder_weights(mg: &MmapGguf) -> HashMap<String, Vec<f32>> {
    deepseekocr::import::encoder_weights(mg).unwrap_or_else(|e| panic!("mmproj import: {e}"))
}
