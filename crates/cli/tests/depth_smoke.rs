...
//! P5 smoke: `brain depth --image --headless` runs end to end and is deterministic.
//!
//! Env-gated on the real checkpoint (ZIPDEPTH_PTH). Builds a synthetic PPM, runs
//! the `brain` binary through the depth image path, and asserts it writes a
...
fn depth_image_headless_is_deterministic() {
    let Ok(ckpt) = std::env::var("ZIPDEPTH_PTH") else {
        eprintln!("SKIP: set ZIPDEPTH_PTH to run the depth smoke test");
        return;
    };
    let dir = std::env::temp_dir().join("brain_depth_smoke");
    ...
    let run = || -> String {
        let o = Command::new(bin())
            .args([
                "depth", "--image", img.to_str().unwrap(),
                "--weights", &ckpt, "--headless", "--out", out.to_str().unwrap(),
            ])
            .env("BRAIN_DEVICE", "cpu")
            .env("DISPLAY", "")
            .output()
            .expect("run brain depth");
        assert!(o.status.success(), "brain depth failed: {}", String::from_utf8_lossy(&o.stderr));
    ...
    let h1 = run();
    let h2 = run();
    assert_eq!(h1, h2, "the depth pipeline must be deterministic across runs");
