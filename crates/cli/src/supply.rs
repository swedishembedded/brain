...
//!
//! Honest scope note: `ensure` only completes a plan that resolves to pure
//! `Download` steps -- today, a quant ref whose upstream `-GGUF` sibling
//! repo already has the file (`crates/modelstore`'s resolution ladder step
//! 2). A plan needing local conversion or local quantization (every base
//! ref, and a quant ref with no upstream artifact) is real but not yet
//! automated end-to-end here -- wiring every model family's importer
//! (`crates/*/src/import.rs`) into one generic dispatch, and driving
//! `checkpoint::quant`/`gguf_write` for the local-quantize step, is
//! follow-up work. `ensure` fails cleanly with which steps are missing
//! rather than silently producing a wrong checkpoint.

use std::collections::HashMap;
use std::sync::{Arc, Condvar, Mutex};

use brain_modelref::ModelRef;
use brain_modelstore::{Hub, Store};
use residency::{Executor, ModelSupplier, Supply};

// Not yet constructed by any live entry point: the natural call site is a
// server surface's auto-fetch path (an HTTP handler classifying an unknown
// `model` before dispatch, D-Bus's `Run`/`Subscribe`), and any change to
// those surfaces requires the full watertight-API security pass (AGENTS.md)
// before it counts as done -- deliberately not attempted in this change.
// This type is complete and tested (see `tests` below); wiring it in is
// follow-up work.
#[allow(dead_code)]
#[derive(Clone)]
enum FetchState {
...
impl StoreSupplier {
    ...
    fn do_ensure(&self, model: &str, exec: &Executor, progress: &mut dyn FnMut(&str, u32, u32)) -> Result<(), String> {
        ...
        if !deferred.is_empty() {
            return Err(format!(
                "{model}: needs {} additional step(s) (convert/local-quantize) that auto-fetch does not automate yet -- fetch and convert manually",
                deferred.len()
            ));
        }
...
mod tests {
    ...
    #[test]
    fn ensure_fails_cleanly_when_the_plan_needs_conversion() {
        ...
        let supplier = StoreSupplier::new(store("supply-test-needs-convert"), Box::new(hub));
        let e = exec();
        let err = supplier.ensure("Qwen/Qwen3-0.6B", &e, &mut |_, _, _| {}).unwrap_err();
        assert!(err.contains("additional step"), "{err}");
    }
