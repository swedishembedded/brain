...
fn kernel_list() -> &'static [(&'static str, &'static str)] {
    ...
    LIST.get_or_init(|| {
        ...
        // B8: `Ops::REQUIRED_KERNELS` grew by the embed/moe_linear_gated
        // storage tiers - additive, mechanical, same fix this file's own
        // comment above already flags for the bf16/f16 trio (`Ops::new`
        // requires the FULL façade kernel set even for a test, like this
        // one, that only exercises `F32`/`I8`/`Q4`).
        ...
        let f16_moe =
            kernels::template::dtype_variant("moe_linear_gated", kernels::MOE_LINEAR_GATED, "w", Dtype::F16)
                .unwrap();
        // B9: paged-KV append (write direction)/scores/apply (read
        // direction) bf16 tiers - additive, mechanical, same fix this file's
        // own comment above already flags for the B8 trio.
        ...
        .unwrap();
        // B10: matmul_dx's bf16-weight-read backward variant. matmul_dw has
        // no bf16 variant at all (it never reads the weight).
        let bf16_matmul_dx =
