...
use deepseekv2::{DeepseekV2, DeepseekV2Config};
...
pub fn expanded(gguf: &std::path::Path, st: &std::path::Path) -> String {
    ...
    let stats = deepseekv2::import::import_file(gguf.to_str().expect("utf-8 path"), &st_s, None)
...
pub fn open(t: u32) -> Option<DeepseekV2> {
    ...
    pin_cpu_backend();
    let cfg = deepseekv2::import::config_from_file(gguf.to_str().expect("utf-8 path"), t).unwrap_or_else(|e| panic!("config_from_file: {e}"));
    ...
    let gpu = gpu_core::testgpu::dev(deepseekv2::PIPELINES);
