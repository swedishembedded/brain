...
//! Face detection + recognition behind the generalized [`capability`] interface
//! — what makes `brain caps facenet` / `brain do facenet detect …`, the D-Bus
//! `Run` method and `brain perf`'s `CapabilityTarget` work with no face-specific
//! plumbing in the CLI or the transports.
//!
//! Two actions, mirroring the two halves of the antelopev2 stack:
//!
...
//! (`capability::blob`). [`unit_norm`] re-expresses the SAME per-model constants
//! in those units — `(255·x − mean)/std = (x − mean/255)/(std/255)` — so there
//! is one pair of constants ([`Preprocess`]), one normalise kernel
//! (`film_chan`, via `imaging::Ctx`), and no second copy of the arithmetic.
...
use crate::config::{ArcFaceConfig, Preprocess, ScrfdConfig};
use crate::detect::Face;
use crate::model::{ArcFace, Scrfd};
...
pub fn manifest() -> Manifest {
    Manifest::new(
        MODEL,
        "Face recognition (insightface antelopev2): SCRFD detection + ArcFace 512-d identity embeddings.",
        vec![detect_spec(), embed_spec()],
    )
}

// ===================== the shared work =====================

/// Both halves of the stack on one device — the single implementation of
/// `detect`/`embed`, shared by the [`FacenetProvider`] and the residency adapter
/// (`crates/cli/src/resident_facenet.rs`).
pub struct Session {
    gpu: Gpu,
    scrfd: Scrfd,
    arcface: ArcFace,
...
impl Session {
    /// Import both antelopev2 ONNX graphs from `dir` and build them on `gpu`.
    /// `dir` comes from a CLI flag or `BRAIN_FACENET_DIR`, never a baked-in path.
    pub fn load(dir: &str, gpu: Gpu) -> Result<Session, String> {
        let (arc, scr) = crate::import::import_dir(std::path::Path::new(dir))?;
        let (scfg, acfg) = (ScrfdConfig::scrfd_10g_bnkps(), ArcFaceConfig::iresnet100());
        // Both models share ONE device handle (`Gpu::share`) — a second `Gpu`
        // per model is the deadlock AGENTS.md forbids.
        let scrfd = Scrfd::new(gpu.share(), scfg.clone(), &scr);
        let arcface = ArcFace::new(gpu.share(), acfg.clone(), &arc);
        Ok(Session { gpu, scrfd, arcface, scfg, acfg })
    }
    ...
    fn det_canvas(&self, chw: &[f32], w: u32, h: u32) -> (Vec<f32>, f32) {
        ...
        let src = ctx.upload("facenet.caps.src", chw);
        ...
        let norm = ctx.normalize(&canvas, cshape, &unit_norm(&self.scfg.pre));
    ...
    pub fn faces(&self, chw: &[f32], w: u32, h: u32) -> Vec<Face> {
        ...
        let mut faces = crate::detect::decode(&self.scfg, &t.out_score, &t.out_bbox, &t.out_kps, det_scale);
    ...
    pub fn run(&self, action: &str, inv: &Invocation) -> ActionResult {
        match action {
            ...
            other => Err(format!("facenet: unknown action '{other}'")),
        }
    }
}
...
/// The executable face stack behind the manifest. Construction is free — the two
/// ONNX graphs import lazily on the first call and stay resident.
pub struct FacenetProvider {
    dir: String,
    hot: Arc<Mutex<Option<(String, Session)>>>,
}

impl FacenetProvider {
    /// `dir` holds `glintr100.onnx` + `scrfd_10g_bnkps.onnx` (the antelopev2
    /// release layout).
    pub fn new(dir: impl Into<String>) -> FacenetProvider {
        FacenetProvider { dir: dir.into(), hot: Arc::new(Mutex::new(None)) }
    }
    /// The two released files this stack needs, by their antelopev2 names.
    pub const RELEASE_FILES: [&str; 2] = ["glintr100.onnx", "scrfd_10g_bnkps.onnx"];

    /// `BRAIN_FACENET_DIR` — `None` when unset or when the directory does not
    /// hold both released graphs.
    pub fn from_env() -> Option<FacenetProvider> {
        let dir = std::env::var("BRAIN_FACENET_DIR").ok().filter(|p| !p.is_empty())?;
        let d = std::path::Path::new(&dir);
        FacenetProvider::RELEASE_FILES.iter().all(|f| d.join(f).exists()).then(|| FacenetProvider::new(dir))
    }
}

impl Provider for FacenetProvider {
    fn manifest(&self) -> Manifest {
        manifest()
    }
    fn action(&self, name: &str) -> Option<Arc<dyn Action>> {
        matches!(name, "detect" | "embed")
            .then(|| Arc::new(FaceAction { name: name.to_string(), dir: self.dir.clone(), hot: self.hot.clone() }) as Arc<dyn Action>)
    }
}
...
impl Action for FaceAction {
    ...
    fn run(&self, inv: &Invocation, _progress: &mut dyn FnMut(Progress)) -> ActionResult {
        let mut guard = self.hot.lock().map_err(|_| "facenet: hot model lock poisoned")?;
        if !matches!(&*guard, Some((d, _)) if *d == self.dir) {
            ...
            *guard = Some((self.dir.clone(), Session::load(&self.dir, gpu)?));
        }
...
mod caps_tests {
    ...
    #[test]
    fn manifest_declares_detect_and_embed() {
        let m = manifest();
    ...
    /// `(255·x − mean)/std` and `(x − mean/255)/(std/255)` must agree, and the
    /// two models' constants must stay different (127.5 vs 128.0).
    #[test]
    fn unit_norm_is_the_same_affine_in_zero_to_one_units() {
        for pre in [ArcFaceConfig::iresnet100().pre, ScrfdConfig::scrfd_10g_bnkps().pre] {
            let n = unit_norm(&pre);
        ...
        assert_ne!(unit_norm(&ArcFaceConfig::iresnet100().pre).std, unit_norm(&ScrfdConfig::scrfd_10g_bnkps().pre).std);
    }
    ...
    fn missing_weights_is_a_clean_error() {
        ...
        reg.register(Arc::new(FacenetProvider::new("/nonexistent/antelopev2")));
        let img = Blob::new(Media::Image, vec![0u8; 12]).with_meta(json!({"w":1,"h":1,"c":3}));
