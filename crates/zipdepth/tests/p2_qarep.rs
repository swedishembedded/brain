...
impl Fix {
    ...
    fn loss(&self, train: bool, r: &[f32]) -> f32 {
        let ctx = Ctx::new(&self.gpu, zipdepth::net::ids());
...
fn fused_conv_reproduces_the_blocks_own_eval_forward() {
    ...
    let br3 = zipdepth::Branch { weight: &w3, gamma: &g3, beta: &b3, run_mean: &m3, run_var: &v3 };
    let br1 = zipdepth::Branch { weight: &w1, gamma: &g1, beta: &b1, run_mean: &m1, run_var: &v1 };
    let (k, bias) = zipdepth::fuse_qarep(&br3, &br1, in_shape.c as usize, cout as usize, 1, true);
