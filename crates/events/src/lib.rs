...
/// Standard base64 (RFC 4648). Moved to `data::base64` - `brain-data` is a
/// layer-3 crate and depending on `brain-events` for this one codec function
/// pulled `brain-forecast` (layer 4) transitively into every leaf crate that
/// depends on `brain-data` (`brain-promote`, `brain-rlcd`), which
/// `scripts/gates/check-crate-layers.sh` exists to catch. Re-exported
/// unchanged so every existing caller of `events::base64::*` is unaffected.
pub use data::base64;
...
mod tests {
    ...
    // base64 roundtrip/error-path tests moved to crate::base64 (now
    // data::base64) alongside the implementation - see that module's own
    // #[cfg(test)] block.
