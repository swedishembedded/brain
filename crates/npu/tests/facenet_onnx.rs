...
//! facenet on the NPU: ArcFace needs **no topology**, SCRFD needs shape freezing.
//!
...
fn weights_dir() -> Option<PathBuf> {
    let p = std::env::var("BRAIN_FACENET_DIR")
...
fn arcface_is_npu_ready_as_shipped() {
    let Some(dir) = weights_dir() else {
        eprintln!("SKIP: set BRAIN_FACENET_DIR to an antelopev2 directory");
        return;
    };
