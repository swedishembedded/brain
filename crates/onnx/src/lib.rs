...
//! serialized model back into tensors + nodes (used by the face crates, whose
//! reference release ships ONNX only), and [`walk`] turns that into a
//! positional, coverage-checked weight import. It stops short of a
//! `Graph::from_proto` - see [`read`] for why.
//!
