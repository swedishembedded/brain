...
use deepseek2ocr::rows::{RowPlan, Src, ViewGrid};
use deepseek2ocr::{DeepseekOcr, DeepseekOcrConfig};
...
fn build_init(cfg: &DeepseekOcrConfig, ck: &HashMap<String, StTensor>) -> HashMap<String, Vec<f32>> {
    ...
    // (`deepseek2ocr::layout`'s unit tests and `tests/real_weight_layout.rs`).
...
/// through `deepseek2ocr::train::lora_init_map` -- the checkpoint coverage
...
/// and fills it through `deepseek2ocr::layout::RowGather`. What this asserts is
...
fn the_real_layout_splices_the_learned_rows_verbatim_and_trains_them() {
    ...
    // tests (`deepseek2ocr::layout`) check the gather in isolation at rel ~1e-5;
