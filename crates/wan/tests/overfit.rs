...
//! WHOLE tiny model rather than one block.
//!
//! `block_grad.rs` and `gradcheck::check_wan` prove the analytic
//! gradients are *correct*; this proves they are *usable for optimization* -
//! Adam over every parameter drives the flow-matching velocity MSE toward zero
//! on a single fixed batch. Running it whole-model is what makes it cover the
//! conditioning path too: `time_embedding`, `time_projection`, every block's
//! `modulation` and `head.modulation` only ever receive gradient through the
//! folds, so a partially-wrong fold shows up here as a loss that stalls.

use wan::modelgrad::{grad_views, grads, init_model, make_flow_batch, params_mut, Cfg};
...
fn the_whole_model_overfits_one_batch() {
    ...
    let x0: Vec<f64> = (0..cfg.latent_len()).map(|i| ((i % 19) as f64 / 19.0 - 0.5) * 1.2).collect();
    let noise: Vec<f64> = (0..x0.len()).map(|i| ((i % 11) as f64 / 11.0 - 0.5) * 0.7).collect();
    let rows = cfg.text_len - 2;
    let ctx: Vec<f64> = (0..rows * cfg.text_dim).map(|i| ((i % 5) as f64 / 5.0 - 0.5) * 1.5).collect();
    let b = make_flow_batch(&cfg, &x0, &ctx, rows, 0.4, &noise);
