...
//! Adapter-only save/load for a trained Qwen3.5-35B-A3B LoRA — a direct port of
//! `qwen3::lora`'s design onto `Qwen35`.
//!
...
/// base — to `path`, carrying a `ModelCard` with `variant_of: base_id` and an
/// `Adapter` descriptor (rank/alpha/targets/dataset_id) so the adapter is
...
pub fn save_adapter(path: &str, model: &Qwen35, card_id: &str, base_id: &str, dataset_id: Option<&str>) -> std::io::Result<()> {
    ...
    let mut card = ModelCard::new(card_id, "qwen35");
...
/// `blocks.0.linear_attn.in_proj_qkv.weight`); this only reads the
/// `.lora_a`/`.lora_b` pair alongside it and adds the low-rank delta.
...
/// uses, and what `Qwen35::lora_fwd`'s unfolded forward computes.
