...
fn indexer_allpass_equals_dense() {
    ...
    let init = glmdsa::init_weights(&cfg_idx, 5);
    let a = Glm::new_on(gpu_core::testgpu::dev(glmdsa::model::PIPELINES), cfg_idx, 2, 6, &init);
    let (x, y) = batch();
    a.set_batch(&x, &y);
    let la = a.forward();
    ...
    let b = Glm::new_on(gpu_core::testgpu::dev(glmdsa::model::PIPELINES), cfg_dense, 2, 6, &init_dense);
    b.set_batch(&x, &y);
    let lb = b.forward();
...
fn indexer_sparse_restricts_attention() {
    ...
    let init = glmdsa::init_weights(&cfg_sparse, 9); // param_list identical for both
    let (x, y) = batch();

    let ms = Glm::new_on(gpu_core::testgpu::dev(glmdsa::model::PIPELINES), cfg_sparse, 2, 6, &init);
    ms.set_batch(&x, &y);
    let ls = ms.forward();
    let mf = Glm::new_on(gpu_core::testgpu::dev(glmdsa::model::PIPELINES), cfg_full, 2, 6, &init);
    mf.set_batch(&x, &y);
    let lf = mf.forward();
...
/// validated on a controlled target in `glmdsa::distill` unit tests.
#[test]
fn indexer_distillation_updates_weights() {
    ...
    let init = glmdsa::init_weights(&cfg, 5);
