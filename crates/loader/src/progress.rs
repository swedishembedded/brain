...
//! Fetch progress rendering -- moved out of `crates/cli/src/pull_cli.rs`,
//! where `brain pull` and [`crate::supply`]'s default-checkpoint auto-fetch
//! both drew from it (the module doc there put it plainly: "making `brain
//! pull` the explicit spelling of the operation auto-fetch already performs
//! is the whole point" -- two code paths that render a fetch's progress would
//! be two sets of bugs, same as two that fetch). `crates/cli/src/pull_cli.rs`
//! now re-exports these instead of owning them, and its own `run_pull` (argv
//! parsing, `USAGE` text) is the one thing that stays there -- genuinely
//! CLI-specific, unlike everything in this module.
//!
//! Two reporting shapes, chosen by whether the destination stream is a
//! terminal: an apt-style bar redrawn in place with `\r` on a terminal, or
//! ten plain greppable lines for the whole transfer when piped. See
//! [`Reporter`]'s own doc for the split.

use std::io::Write;
use std::time::{Duration, Instant};

use brain_modelstore::Remaining;

/// How many progress lines a whole piped transfer is allowed to spend.
...
/// across the WHOLE transfer, not within one file, and answers with the new
...
/// tenth of the transfer) reports only the highest, so the budget is a
/// ceiling: never more than `lines` lines, sometimes fewer for a very small
/// model.
...
impl LineBudget {
    pub fn new(total: u64, lines: u32) -> LineBudget {
    ...
    pub fn tick(&mut self, cumulative: u64) -> Option<u32> {
...
/// Renders a fetch's progress onto one stream.
///
...
pub struct Reporter<'a> {
    ...
    remaining: Remaining,
    budget: LineBudget,
    /// Bytes belonging to files already finished in this transfer.
    done_before: u64,
...
impl<'a> Reporter<'a> {
    pub fn new(mode: Mode, out: &'a mut dyn Write, label: impl Into<String>, remaining: Remaining) -> Reporter<'a> {
        let now = Instant::now();
        Reporter {
            mode,
            out,
    ...
    pub fn on_bytes(&mut self, file: &str, got: u64, _total: Option<u64>) {
...
/// bar stuck at zero.
pub fn render_bar(label: &str, done: u64, total: u64, rate: Option<f64>, eta: String, width: usize) -> String {
    if total == 0 {
...
pub fn human_eta(bytes_left: u64, rate: Option<f64>) -> String {
    match rate {
...
mod tests {
    ...
    fn scratch(name: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("brain-loader-progress-test-{name}"));
    ...
    /// what keeps a piped transfer at ten lines rather than one per chunk.
    #[test]
    ...
    /// whole transfer is many transfer chunks wide.
    ...
    /// TOTAL bytes of the transfer, so a six-file model still costs ten
    /// progress lines -- not ten per file. A per-file counter would print
    /// sixty here and is the mistake most easily made.
    #[test]
    fn pipe_mode_spends_ten_lines_on_the_whole_transfer_not_ten_per_file() {
        let dir = scratch("pipe-budget");
        let hub = six_file_hub();
        let (out, files) = run_capture(&dir, &hub, Mode::Pipe);
        assert_eq!(files, 6, "the fixture must exercise a MULTI-file transfer");
        let progress: Vec<&str> = out.lines().filter(|l| l.contains('%')).collect();
        assert_eq!(progress.len(), 10, "expected ten progress lines for the whole transfer, got {}:\n{out}", progress.len());
        ...
        // the whole transfer, not of whichever file is in flight.
        let pcts: Vec<u32> = progress.iter().filter_map(|l| l.split('%').next()?.rsplit(|c: char| !c.is_ascii_digit()).next()?.parse().ok()).collect();
        assert_eq!(pcts, (1..=10).map(|i| i * 10).collect::<Vec<u32>>(), "percentages of the whole transfer, in order:\n{out}");
        std::fs::remove_dir_all(&dir).ok();
    }
