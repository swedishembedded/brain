...
use yolo::boxmath::xywhn_to_xyxy;
use yolo::{init_weights, Detection, GtBox, LossMode, Yolo, YoloConfig};
...
pub fn detect_split_train(
    ...
    for img in 0..n {
        ...
        for i in 0..a {
            ...
            let bx = yolo::boxmath::dist_to_xyxy(d, an.ax, an.ay, an.stride);
            dets.push([bx[0], bx[1], bx[2], bx[3], best_s, best_c as f32]);
        }
        out.push(yolo::nms(&dets, iou_thresh, 300));
    }
    let _ = side;
    out
}
