...
fn import_round_trips_a_synthetic_checkpoint() {
    ...
    let got = unet::import::remap(src.clone(), &cfg).expect("remap");
    ...
    assert!(unet::import::remap(extra, &cfg).is_err(), "an unused source tensor was accepted");
    // ... and so must a missing one.
    let mut missing = src;
    missing.remove("conv_in.weight");
    assert!(unet::import::remap(missing, &cfg).is_err(), "a missing source tensor was accepted");
}
