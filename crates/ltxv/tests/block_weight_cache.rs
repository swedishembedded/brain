...
fn cached_forward_is_bit_identical_to_an_uncached_one() {
    ...
    let fresh_a = RefCell::new(Vec::new());
    ...
    let fresh_b = RefCell::new(Vec::new());
    ...
    assert_eq!(shared.borrow().len(), cfg.num_layers as usize);
    assert!(shared.borrow().iter().all(|slot| slot.is_some()), "every layer must be cached after the first (cache-miss) forward");
