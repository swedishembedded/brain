...
//! no intermediate value at all - the only thing that reads both is the fold
...
impl DevicePlan {
    ...
    /// current selection.
    ///
    /// `device` is `GenOpts::device` (`Some("cpu")` / `Some("gpu")` /
    /// `None`) - a CPU-backend run has no cards to split across and resolves
    /// to [`Placement::single`] regardless of what the plan asked for, rather
