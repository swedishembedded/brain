...
pub fn manifest() -> Manifest {
    ...
    .param(ParamSpec::new("mid_frame", ParamType::Str, "optional path to a PNG/JPEG still, held fixed at ONE INTERIOR instant of the clip, so a single generation can be pinned at its start, its middle and its end at once. The same appended VideoConditionByKeyframeIndex guide end_frame uses, at an arbitrary pixel frame. Same eta/dit_config requirements as start_frame."))
