...
//! Qwen3.8-27B's capabilities behind the generalized [`capability`]
//! interface - what makes `brain caps qwen35` / `brain do qwen35 generate
//! ...` work with no qwen35-specific plumbing in the CLI. Mirrors
//! `qwen35moe::caps` exactly (one streaming `generate` action, the same
//! `ParamSpec` list, a lazily-constructed resident model cached across calls
//! and keyed by weights path + built context capacity): see that module's
...
//!   ever builds a plain full-parameter fp32 `Qwen35`). See
//!   `crates/qwen35/src/serve.rs`'s own module doc for the complete
//!   "deliberately deferred" list this action does not attempt.
...
/// The model id used on the CLI (`brain do qwen35 ...`) and the event API. A
/// `brain/`-prefixed synthetic id (this crate's checkpoints carry no
/// upstream vendor/repo provenance of their own here) - the real upstream id
/// is `Qwen/Qwen3.8-27B-FP8`, used where a checkpoint's actual provenance
...
/// The resident (hot) model: the loaded inference graph plus the key that
/// fixes it. Reused while the weights path matches and the built context
        ...
        let plan = match &tok {
            ...
            None => {
                ...
                let ids: Vec<u32> = prompt
                    ...
                    .filter(|s| !s.is_empty())
                    .map(|s| s.parse::<u32>().map_err(|_| format!("qwen35 generate: without a tokenizer the prompt must be token ids (got '{s}')")))
                    .collect::<Result<_, _>>()?;
                if ids.is_empty() {
                    return Err("qwen35 generate: empty prompt".to_string());
                }
                let eos: Vec<u32> = match inv.get_i64("eos") {
                    Some(e) if e >= 0 => vec![e as u32],
                    _ => Vec::new(),
...
mod tests {
    ...
    fn tiny_checkpoint_generates_with_per_token_progress() {
        ...
        let tensors: Vec<(String, Vec<u64>, Vec<f32>)> = cfg
            ...
            .collect();
        let dir = std::env::temp_dir().join(format!("qwen35-caps-e2e-{}", std::process::id()));
    ...
    /// self-skips loudly when unset. The checkpoint's vocab is sized to the
    /// real tokenizer's full range so the rendered chat-template special
    /// tokens (`<|im_start|>` etc., ids up in the 151000s) never index
    ...
    fn tokenizer_present_runs_the_shared_chat_parse_with_tools() {
        ...
        let tensors: Vec<(String, Vec<u64>, Vec<f32>)> = cfg
            ...
            .collect();
        let dir = std::env::temp_dir().join(format!("qwen35-caps-chat-e2e-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("tiny.safetensors");
        checkpoint::save(path.to_str().unwrap(), cfg.to_json(), &tensors);
