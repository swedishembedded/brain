...
//! P0 smoke + parity test for `qwen35::q8`'s int8 (DP4A) inference path:
...
//! fp32 computation `crates/qwen35/tests/model_smoke.rs` already exercises,
...
//! `crates/qwen35/src/q8.rs`'s own module doc for the full rationale). At the
...
/// (docs/lessons.md #4), and `full_attention_interval=4`/`n_layers=8`
...
fn qwen35_q8_build_resident_shape_matches_the_designed_coverage() {
    ...
    let cfg = tiny_i8_cfg();
    let init = qwen35::init::init_weights(&cfg, 5);
    ...
    for (l, ty) in types.iter().enumerate() {
        match (ty, &q8.mixers[l]) {
            (LayerType::Linear, qwen35::q8::Q8Mixer::Gdn(_)) => {}
            (LayerType::Full, qwen35::q8::Q8Mixer::Gqa(_)) => {}
...
/// default-backend variants below (`docs/lessons.md` #5).
...
/// (`docs/lessons.md` #5 asks for both backends; see
...
/// unique to qwen35 either: `qwen::q8::Q8`'s own int8 tier has NO test
/// anywhere in this repo that actually EXECUTES its forward on the CPU
/// backend — `crates/qwen/tests/flops.rs`'s `i8_model_reports_int_ops` builds
/// an int8 `Qwen` under `Backend::Cpu` but only walks its OFFLINE step list
/// via `cost_fwd()`, never calls `.forward()`.
///
/// Rather than silently skip the CPU leg `docs/lessons.md` #5 asks for, this
