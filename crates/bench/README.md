...
- Depends on `brain-data` (datasets/loader), `brain-gpt2` (the model trained
...
| architecture registry | `src/arch.rs` | named architectures (`gpt2`, `gpt2-small`, `gpt2-wide`) + size descriptors |
...
immediately: `gpt2` (size per benchmark), `gpt2-small` (fixed 1-layer / d_model 32),
`gpt2-wide` (fixed 2-layer / d_model 96). A fixed-size variant is a `DecoderLm`
...
Example (abridged - a real `make bench/eval ARCH=gpt2 SEED=1234` run, the
calibrated battery; `results/gpt2-1234.json` is regenerated by that command):
...
The full `gpt2` battery passes **10/10** gating benchmarks at this seed (mod_add is
informational). Regenerate it with `make bench/eval ARCH=gpt2 SEED=1234` for the
complete file.
...
```
metric                       gpt2@c2829fb gpt2-small@c2829fb
----------------------------------------------------------
params                            110336             17888
...
```bash
BRAIN_DEVICE=cpu make bench/scale ARCH=gpt2              # or:
./target/release/brain bench scale --arch gpt2 --seed 1234
```
...
```bash
./target/release/brain bench advise results/gpt2-1234.json results/scale-gpt2-1234.json
BRAIN_DEVICE=cpu make bench/advise ARCH=gpt2            # finds both artifacts
```
...
verdict, `train_ce`, score/Mparam, lever). In the worked `gpt2` run above the
...
BRAIN_DEVICE=cpu make bench/scale ARCH=gpt2    # per-capability size sweep -> scale-<arch>-<seed>.json
BRAIN_DEVICE=cpu make bench/advise ARCH=gpt2   # ranked tuning recommendations
```
