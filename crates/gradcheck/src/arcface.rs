...
//! `check_arcface` - the finite-difference gate on `crates/arcface`'s ArcFace
//! training graph.
//!
//! # What this gates
//!
//! A tiny IResNet ([`arcface::train::ArcFaceTrainConfig::tiny`]: 5 residual
...
//!   carries no recognition gradient. `crates/scrfd` has no backward at all and
...
//!
//! `arcface` is a device model composed from shipped WGSL, so this is
...
//! `arcface` does not implement `model::Model` (there is no image/label `Batch`
//! variant), so this is a direct `CheckModel` impl, as in
//! `crates/yolo/tests/p2_blocks.rs`.

use data::rng::Rng;
use arcface::train::{ArcFaceTrainConfig, ArcFaceTrainer};

use crate::{directional_check, CheckModel, Report};

/// Newtype so the `CheckModel` impl can live here - `CheckModel` is this
/// crate's trait and `ArcFaceTrainer` is `arcface`'s type, so the impl has to
