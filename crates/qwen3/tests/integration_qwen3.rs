...
fn qwen3_inference_coherent() {
    ...
    let out = qwen3::sample::generate(&m, &ids, 4, 0.0, 0, 1.0, None, &mut rng); // greedy
...
fn qwen3_shard_real_2gpu() {
    ...
    let pipe = qwen3::Pipeline::<Qwen>::new(cfg.clone(), b, t, &init, &[0, 1]);
