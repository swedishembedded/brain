...
//! Qwen3's [`ArchSpec`]: which on-disk artifacts satisfy its `weights` and
//! `tokenizer` roles.
//!
//! `weights` accepts EITHER a brain-format `.safetensors` checkpoint
//! (`ModelCard.family == "qwen"` - the family string every Qwen3 checkpoint
//! in this workspace already writes, e.g. `crate::import::convert`'s own
//! `ModelCard::new(id, "qwen")`, unrelated to `crates/qwen35`'s own
//! `"qwen35"` family) or a `general.architecture == "qwen3"` GGUF
//! (`crate::gguf_import::GGUF_ARCHITECTURE`) - the same shipped-release-vs-
//! locally-trained-checkpoint duality `crates/qwen35/src/spec.rs::Qwen35Spec`
//! already established for its own architecture, mirrored here rather than
//! reinvented.
//!
...
//! follows. A GGUF `weights` candidate that carries its own embedded
//! `tokenizer.ggml.*` KVs self-satisfies the role with no sibling file
//! needed - the same real-world gap `Qwen35Spec`'s own doc explains fixing
//! (a bare GGUF release with no sibling `tokenizer.json` otherwise could
//! never resolve at all).
//!
//! This is the prerequisite [`crates/sdk`]'s `TextGenerationPipeline` names
//! as a real, tracked gap: without an `ArchSpec` here, that pipeline has
//! nothing to resolve a hub id against and stays scoped to a literal local
//! checkpoint path. Writing this spec does not change that pipeline's
//! existing local-path constructor - it adds the seam a future
//! `from_pretrained(hub_id)` on it would resolve through, the same
//! `weights`/`tokenizer` role pair `crates/qwen35`'s own resident/CLI
//! consumers already read via `Qwen35Spec`.
//!
...
impl ArchSpec for Qwen3Spec {
    ...
    fn classify(&self, records: &[ArtifactRecord], inventory_root: &Path) -> Vec<(usize, String, Confidence)> {
        ...
        // A GGUF release carries its own `tokenizer.ggml.*` KVs, so it never
        // NEEDS a sibling `tokenizer.json` to satisfy this role - only
        // self-satisfy for a GGUF whose vendor directory has no external
        // tokenizer.json already classified above, so a real sibling
        // tokenizer.json (this arch's other real-world shape) stays the
        // sole, unambiguous candidate rather than tying with the GGUF
        // itself. Mirrors `Qwen35Spec::classify`'s own tail exactly.
    ...
    fn validate(&self, assembly: &Assembly) -> Result<(), String> {
        let weights = assembly.roles.get("weights").ok_or("qwen3 validate: assembly has no weights role")?;
        let tokenizer = assembly.roles.get("tokenizer").ok_or("qwen3 validate: assembly has no tokenizer role")?;
...
mod tests {
    ...
    fn tmp(tag: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!("brain-qwen3-spec-test-{tag}-{}-{n}", std::process::id()))
    }
    ...
    fn write_safetensors(path: &Path, family: &str) {
        std::fs::create_dir_all(path.parent().unwrap()).unwrap();
        let card = checkpoint::st::ModelCard::new("brain/qwen3", family);
    ...
    fn resolves_cleanly_from_a_gguf_and_its_sibling_tokenizer() {
        ...
        let gguf_path = vendor.join("Qwen3-4B-Q8_0.gguf");
    ...
    /// A bare GGUF with NO sibling `tokenizer.json` at all still resolves:
    /// the file carries its own `tokenizer.ggml.*` KVs.
    #[test]
    ...
    fn classify_uses_gguf_header_content_not_filename() {
        ...
        let real = dir.join("vendor").join("Qwen3-4B-Q8_0.gguf");
        write_gguf(&real, GGUF_ARCHITECTURE, TOY_VOCAB);
        let decoy = dir.join("vendor").join("totally-real-qwen3-checkpoint.gguf");
        write_gguf(&decoy, "some-other-arch", TOY_VOCAB);

        let records = brain_modelstore::inventory::scan(&dir);
        let out = Qwen3Spec.classify(&records, &dir);
    ...
    /// signal, not the file's own name - and NOT qwen35's own family string,
    /// even though both are Qwen-lineage decoders.
    #[test]
    fn classify_recognizes_a_brain_format_safetensors_checkpoint_by_its_card_and_not_qwen35s() {
        let dir = tmp("safetensors-card");
        let real = dir.join("vendor").join("qwen3-run7.safetensors");
        write_safetensors(&real, CARD_FAMILY);
        let decoy = dir.join("vendor").join("unrelated.safetensors");
        write_safetensors(&decoy, "qwen35");
    ...
    fn no_candidates_on_disk_is_missing_not_a_panic() {
        ...
        let spec = Qwen3Spec;
        let specs: Vec<&dyn ArchSpec> = vec![&spec];
        assert!(matches!(resolve("qwen3", &records, &specs, &BTreeMap::new()), Resolution::Missing(_)));
    }
    ...
    fn a_safetensors_only_checkpoint_needs_an_explicit_tokenizer_override() {
        ...
        let weights_path = dir.join("vendor").join("repo").join("qwen3.safetensors");
        ...
        let spec = Qwen3Spec;
        let specs: Vec<&dyn ArchSpec> = vec![&spec];
        assert!(matches!(resolve("qwen3", &records, &specs, &BTreeMap::new()), Resolution::Missing(_)), "vocab-based auto-match should not fire for a bare safetensors checkpoint");

        let mut overrides = BTreeMap::new();
        overrides.insert("tokenizer".to_string(), tok_path.to_string_lossy().into_owned());
        match resolve("qwen3", &records, &specs, &overrides) {
            Resolution::Resolved(assembly) => {
