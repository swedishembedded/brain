...
//
// Record stride 11 in `recs`: {v_xy(2), v_conic(3), v_opacity, v_rgb(3),
// v_depth, id}; record keys (gaussian id) go to `keys` for the radix sort;
// offsets come from the scanned per-pixel counts. dimg is the upstream RGBA
// gradient (dL/d rgb, dL/d alpha_out). One invocation per pixel.
//
// `height` is the number of ROWS this dispatch covers and `y0` the first of
// them (see splat_bwd_count). `dimg` and `ddepth` are the whole frame and
// indexed by absolute pixel; `offsets` is band-local, because the records of
// one band are numbered from zero in a buffer sized for that band alone.
//
// `ddepth` is dL/d(sum_i z_i alpha_i T_i), the UNNORMALIZED accumulated
// depth. The expected depth the forward reports divides that by A, and A is
// the alpha output, so a caller supervising expected depth splits its
// gradient between this buffer and dimg's alpha channel
// (`splat::renderer::add_expected_depth_vjp` is the one place that does it).
// Keeping the division on the host leaves this kernel with the same shape of
// walk the colour already has: the depth suffix S_d behaves exactly like the
// colour suffix, so v_alpha gains one more term and nothing else moves.
...
@group(0) @binding(7) var<storage, read_write> recs:    array<f32>; // cap*11 (+ keys via bitcast slot 10)
@group(0) @binding(8) var<storage, read>       ddepth:  array<f32>; // W*H
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>,
    ...
    // Absolute pixel: the band restricts which rows are WALKED, while the
    // upstream gradients are the whole frame's.
    let abs = py * p.width + px;
    let vr = dimg[abs * 4u];
    let vg = dimg[abs * 4u + 1u];
    let vb = dimg[abs * 4u + 2u];
    let va_out = dimg[abs * 4u + 3u];
    let vd = ddepth[abs];
    ...
    for (var j = start; j < end; j = j + 1u) {
        ...
        // (suffix colors + background + the alpha output). Depth composites by
        // the same weights, so it enters the same way with z in place of rgb.
        ...
        recs[r + 7u] = w * vg;
        recs[r + 8u] = w * vb;
        recs[r + 9u] = w * vd;            // d(accumulated depth)/d z_i = w
        recs[r + 10u] = bitcast<f32>(g);  // gaussian id (sort key source)
        pos = pos + 1u;
        t = next_t;
    }
}
