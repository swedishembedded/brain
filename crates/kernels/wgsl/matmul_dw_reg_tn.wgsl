...
//
// Identical to `matmul_dw_reg` except for how operand A is indexed. That one
// takes dY as [m, n] and reads `a[gk*n + ar]`, where `gk` (the contraction
// index) is what varies fastest across threads — so consecutive lanes land a
// full row apart and the A-side load costs one memory transaction per lane.
// This one takes dY^T as [n, m] and reads `a[ar*m + gk]`, making those same
// lanes contiguous: 8 lanes per 32-byte sector instead of 8 sectors.
//
// The transpose is FREE for conv backward: dY arrives NCHW, which is already
// [Cout, HW] = dY^T. `blocks::grad` builds the [HW, Cout] view for `bias_grad`
// and the dX GEMM anyway, and used to hand that copy here too — paying for a
// transpose in order to make this kernel's loads worse.
//
// NOTE the B side (X = the im2col `col[HW, CinKK]`) is still read with a row
// stride; fixing that needs `im2col_at` to emit col^T. This kernel claims only
// the A half.
//
// @workgroup_size(256).
...
fn main(@builtin(workgroup_id) wgid: vec3<u32>,
    ...
    for (var e = 0u; e < 4u; e = e + 1u) {
        let gk = skk[e];
        { let ar = arow_g[e]; if (ar < R && gk < L) { As[skk[e] * BM + sr[e]] = a[ar * p.m + gk]; } else { As[skk[e] * BM + sr[e]] = 0.0; } }
    ...
    for (var c = 0u; c < nchunks; c = c + 1u) {
        ...
        if (has_next) {
            ...
            for (var e = 0u; e < 4u; e = e + 1u) {
                let gk = k1 + skk[e];
                { let ar = arow_g[e]; if (ar < R && gk < L) { rA[e] = a[ar * p.m + gk]; } else { rA[e] = 0.0; } }
