...
// scale (`sx`, from `model::int8::quant_rows_steps`) times the GROUP-WISE
// weight scale (`sw`, from `model::int8::quantize_weight` - one per 32 int8
// along K, `model::int8::GROUP`).
//
...
//
// K must be a multiple of 32. Unlike the two GEMV kernels, this one's inner
// loop walks K in order, so a whole group's eight words are consecutive: the
// integer sum runs to the group boundary and only THEN converts and scales -
// one convert + one FMA per 32 weights, not per 4.
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>,
    ...
    for (var gr: u32 = 0u; gr < ng; gr = gr + 1u) {
        let g0 = gr * WPG;
        var acc = 0i;
        for (var j: u32 = 0u; j < WPG; j = j + 1u) {
            acc = acc + dot4I8Packed(xq[x_base + g0 + j], wq[w_base + g0 + j]);
        }
