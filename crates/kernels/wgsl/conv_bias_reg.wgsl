...
// BN-affine+SiLU one. Same math as conv_bias.wgsl; the tiling is what makes
// the integrated-GPU path compute-bound instead of input-read-bound.
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>,
    ...
    let nco = n * p.Cout;
    {
        ...
        { y[row+q0] = a00 + bb; }
        if (v1) { y[row+q1] = a10 + bb; }
        if (v2) { y[row+q2] = a20 + bb; }
        if (v3) { y[row+q3] = a30 + bb; }
    }
    if (1u < nc) {
        let co = co0+1u; let bb = bias[co]; let row = (nco+co)*psz;
        { y[row+q0] = a01 + bb; }
        if (v1) { y[row+q1] = a11 + bb; }
        if (v2) { y[row+q2] = a21 + bb; }
        if (v3) { y[row+q3] = a31 + bb; }
    }
    if (2u < nc) {
        let co = co0+2u; let bb = bias[co]; let row = (nco+co)*psz;
        { y[row+q0] = a02 + bb; }
        if (v1) { y[row+q1] = a12 + bb; }
        if (v2) { y[row+q2] = a22 + bb; }
        if (v3) { y[row+q3] = a32 + bb; }
    }
    if (3u < nc) {
        let co = co0+3u; let bb = bias[co]; let row = (nco+co)*psz;
        { y[row+q0] = a03 + bb; }
        if (v1) { y[row+q1] = a13 + bb; }
        if (v2) { y[row+q2] = a23 + bb; }
        if (v3) { y[row+q3] = a33 + bb; }
    }
    if (4u < nc) {
        let co = co0+4u; let bb = bias[co]; let row = (nco+co)*psz;
        { y[row+q0] = a04 + bb; }
        if (v1) { y[row+q1] = a14 + bb; }
        if (v2) { y[row+q2] = a24 + bb; }
        if (v3) { y[row+q3] = a34 + bb; }
    }
    if (5u < nc) {
        let co = co0+5u; let bb = bias[co]; let row = (nco+co)*psz;
        { y[row+q0] = a05 + bb; }
        if (v1) { y[row+q1] = a15 + bb; }
        if (v2) { y[row+q2] = a25 + bb; }
        if (v3) { y[row+q3] = a35 + bb; }
    }
    if (6u < nc) {
        let co = co0+6u; let bb = bias[co]; let row = (nco+co)*psz;
        { y[row+q0] = a06 + bb; }
        if (v1) { y[row+q1] = a16 + bb; }
        if (v2) { y[row+q2] = a26 + bb; }
        if (v3) { y[row+q3] = a36 + bb; }
    }
    if (7u < nc) {
        let co = co0+7u; let bb = bias[co]; let row = (nco+co)*psz;
        { y[row+q0] = a07 + bb; }
        if (v1) { y[row+q1] = a17 + bb; }
        if (v2) { y[row+q2] = a27 + bb; }
        if (v3) { y[row+q3] = a37 + bb; }
    }
}
