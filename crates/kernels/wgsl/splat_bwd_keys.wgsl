...
// gradient records (the last word of each `stride`-word record holds the id
// bit-cast to f32) and index payloads.
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>,
    ...
    if (i >= p.n) { return; }
    keys[i] = bitcast<u32>(recs[i * p.stride + p.stride - 1u]);
    vals[i] = i;
}
