...
use omni::int8_thinker_resident::Int8ThinkerResident;
...
fn reclaim_events_for_one_forward(cfg: &MoeTextConfig, n: u32, seed: u64) -> u64 {
    ...
    let resident = Int8ThinkerResident::new(path.to_str().unwrap().to_string(), omni::config::ThinkerConfig::defaults().with_text(cfg.clone()), caps);
