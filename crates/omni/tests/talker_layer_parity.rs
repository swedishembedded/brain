...
//! `omni::talker::layer_fwd`: RMSNorm + GQA (QK-norm, table-driven M-RoPE) +
//! sparse MoE FFN + the always-active shared expert
//! (`model::moe::shared_expert_fwd`) — the one architectural difference from
//! `omni::thinker::layer_fwd`. See `crates/omni/src/talker.rs`'s module doc.
//!
...
use omni::talker::{layer_fwd, talker_pipelines, TalkerLayerWeights};
