...
fn real_template_places_image_and_audio_placeholders_inline_after_their_own_captions() {
    ...
    let prompt = omni::caps::render_chat_prompt(&tmpl, &inv).expect("render");
...
fn real_template_renders_the_tools_preamble_when_tools_are_forwarded() {
    ...
    let prompt_no_tools = omni::caps::render_chat_prompt(&tmpl, &inv_no_tools).expect("render");
...
fn typed_array_plus_strip_matches_the_old_always_flatten_prompt_byte_for_byte() {
    ...
    let old_prompt = omni::caps::render_chat_prompt(&tmpl, &old_inv).expect("render old");
    ...
    let new_prompt_raw = omni::caps::render_chat_prompt(&tmpl, &new_inv).expect("render new");
    let new_prompt = omni::mm::strip_media_placeholder_text(&new_prompt_raw);
