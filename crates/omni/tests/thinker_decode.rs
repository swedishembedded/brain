...
//! `omni::thinker::decode` -- tiny-config smoke test proving the full-stack
...
//! parity test (which now also exercises the real `qwenvl::mrope` table this
...
use omni::thinker::{decode, layer_fwd, thinker_pipelines, ThinkerLayerWeights, ThinkerWeights};
...
fn decode_matches_hand_chained_layer_fwd_plus_final_norm() {
    ...
    let mut h = x;
    for lw in &layers {
        let (out, ..) = layer_fwd(&gpu, &cfg, lw, &h, &cos, &sin, n);
        h = out;
    }
