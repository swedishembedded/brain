...
fn generate_through_the_executor_matches_a_direct_activate_multi_reference() {
    ...
    let reference_resident = Int8ThinkerResident::new(path.to_str().unwrap().to_string(), omni::config::ThinkerConfig::defaults().with_text(cfg.clone()), caps_for_split(path.to_str().unwrap(), &cfg, &[Device::Gpu(0), Device::Gpu(1)], 2));
    ...
    let resident = Int8ThinkerResident::new(path.to_str().unwrap().to_string(), omni::config::ThinkerConfig::defaults().with_text(cfg.clone()), caps_for_split(path.to_str().unwrap(), &cfg, &[Device::Gpu(0), Device::Gpu(1)], 2));
