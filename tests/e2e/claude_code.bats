...
run_claude() {
  timeout "$CLAUDE_TIMEOUT" env -u ANTHROPIC_AUTH_TOKEN \
    ...
    ANTHROPIC_MODEL="brain/mock" \
    ANTHROPIC_DEFAULT_HAIKU_MODEL="brain/mock" \
    ANTHROPIC_DEFAULT_SONNET_MODEL="brain/mock" \
    ANTHROPIC_DEFAULT_OPUS_MODEL="brain/mock" \
...
@test "the local Anthropic surface lists the mock model" {
  ...
  [ "$status" -eq 0 ]
  echo "$output" | jq -e '.data | map(.id) | index("brain/mock")' >/dev/null
}

@test "missing key is rejected (401) — never open" {
  run curl -s --max-time 5 -o /dev/null -w "%{http_code}" -X POST \
    -H 'content-type: application/json' \
    -d '{"model":"brain/mock","messages":[{"role":"user","content":"hi"}],"max_tokens":8}' \
...
@test "claude uses OUR key (not the subscription) and reaches the local backend" {
  ...
  echo "$output" | grep -E '"type":"system"' | head -1 | jq -e '.model == "brain/mock"' >/dev/null
}
