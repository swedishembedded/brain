...
const PIPELINES: &[(&str, &str)] = &[
    ...
    ("pos_add", include_str!("shaders/pos_add.wgsl")),
    ("layernorm", include_str!("shaders/layernorm.wgsl")),
    ("ln_stats", include_str!("shaders/ln_stats.wgsl")),
    ("layernorm_dx", include_str!("shaders/layernorm_dx.wgsl")),
    ("layernorm_dgamma", include_str!("shaders/layernorm_dgamma.wgsl")),
    ("layernorm_dbeta", include_str!("shaders/layernorm_dbeta.wgsl")),
    ("matmul", include_str!("shaders/matmul.wgsl")),
    ("bias_add", include_str!("shaders/bias_add.wgsl")),
    ("bias_grad", include_str!("shaders/bias_grad.wgsl")),
    ("attn_scores_masked", include_str!("shaders/attn_scores_masked.wgsl")),
    ("attn_softmax_masked", include_str!("shaders/attn_softmax_masked.wgsl")),
    ...
    ("ce_value_masked", include_str!("shaders/ce_value_masked.wgsl")),
    ("ce_grad_masked", include_str!("shaders/ce_grad_masked.wgsl")),
    ...
    ("pos_bwd", include_str!("shaders/pos_bwd.wgsl")),
    ("emb_bwd", include_str!("shaders/emb_bwd.wgsl")),
    ("add2", include_str!("shaders/add2.wgsl")),
    ("gradnorm_sq", include_str!("shaders/gradnorm_sq.wgsl")),
    ("grad_scale", include_str!("shaders/grad_scale.wgsl")),
    ("adamw", include_str!("shaders/adamw.wgsl")),
    ("clip_coef", include_str!("shaders/clip_coef.wgsl")),
    ("grad_scale_buf", include_str!("shaders/grad_scale_buf.wgsl")),
];
...
impl Pid {
    ...
    pub fn save(&self, path: &str) {
        let tensors: Vec<(String, Vec<u64>, Vec<f32>)> = self
            ...
            .collect();
        crate::checkpoint::save(path, self.cfg.to_json(), &tensors);
    }
}
...
mod tests {
    ...
    fn forward_is_finite_and_deterministic() {
        ...
        cfg.block_size = 16;
        let init = crate::pid_data::init_weights(&cfg, 3);
    ...
    fn backward_runs_and_grads_finite() {
        ...
        let mut cfg = PidConfig::default_small();
        cfg.block_size = 16;
        let init = crate::pid_data::init_weights(&cfg, 4);
        let model = Pid::new(cfg.clone(), 1, 8, &init);
        let x: Vec<u32> = vec![BOS, EV_START, 1, 50, 50, 50, EV_END, DECIDE];
        let mut y = vec![IGNORE; 8];
        y[7] = 30;
    ...
    fn profile_inference_returns_ordered_positive_timing() {
        ...
        let mut cfg = PidConfig::default_small();
        cfg.block_size = 16;
        let init = crate::pid_data::init_weights(&cfg, 5);
