...
//! Tiny sparse-MoE Transformer — inference + generation as a raw-WGSL wgpu
//! compute pipeline.
//!
//! This mirrors the forward pass of `tiny_sparse_moe.py` exactly, stage by
//! stage, with one WGSL compute kernel per stage:
//!
//!   embed -> [ rmsnorm, qkv-matmul, rope, attention, out-matmul, +residual,
//!              rmsnorm, router-matmul, router-gate,
//!              per-expert(gate/up matmul, silu*up, down matmul, weighted-add),
//!              +residual ] x n_layers
//!         -> rmsnorm -> lm_head-matmul -> (CPU) sample.
//!
//! Everything is fp32 and uses only core compute features + a single bind
//! group with <=4 storage buffers, so it runs on any compute device, including
//! old sm_61 (Pascal) via Vulkan. Top-k MoE routing is realised by evaluating
//! all experts densely and masking by the renormalised gate — numerically
//! identical to sparse top-k dispatch (inference has no capacity limit).
//!
//! Usage:
//!   moe --weights moe.weights --prompt 1,2,3,4 --max-new 64 \
//!       [--temperature 0.8] [--top-k 40] [--seed 1234]
...
// ---------------------------------------------------------------------------
// Weight file (written by `tiny_sparse_moe.py export`)
// ---------------------------------------------------------------------------
...
fn load_weights(path: &str) -> Weights {
    ...
    // The float data section follows the JSON header. We decode from
    // little-endian bytes (the byte offset is not guaranteed 4-aligned, so we
    // cannot cast the slice directly to &[f32]).
...
// ---------------------------------------------------------------------------
// Kernel registry (index = order in `pipelines`)
// ---------------------------------------------------------------------------
...
struct Engine {
    ...
    // scratch buffers (sized for the maximum sequence length = block_size)
    tokens: wgpu::Buffer,
    x: wgpu::Buffer,
    xn: wgpu::Buffer,
    qkv: wgpu::Buffer,
...
impl Engine {
    fn new(w: Weights) -> Engine {
        ...
        let (device, queue) = pollster::block_on(adapter.request_device(&wgpu::DeviceDescriptor {
            ...
            // downlevel defaults => broad compatibility (incl. sm_61 / Pascal).
        ...
        // Upload every parameter tensor to its own GPU buffer.
    ...
    /// Build one dispatch: bind group 0 = [uniform, buffers...], grid = ceil(threads/64).
    fn step(
        &self,
        kind: usize,
        buffers: &[&wgpu::Buffer],
    ...
    /// Run the full forward pass over `tokens` and return the logits
    /// (`vocab_size` floats) for the LAST position.
    fn forward(&self, tokens: &[u32]) -> Vec<f32> {
        ...
        for l in 0..c.n_layers {
            ...
            // RoPE on the q region (offset 0) and k region (offset d) of qkv
            ...
            // every expert, combined by its gate (expert 0 sets, rest accumulate)
            for ei in 0..e {
        ...
        // Record one compute pass (wgpu inserts the inter-dispatch barriers).
        let mut enc = self
            .device
        ...
        // copy the LAST position's logits to the mappable staging buffer
...
fn parse_args() -> Args {
    ...
    // allow an explicit `generate` subcommand token
...
fn main() {
    // Subcommands: `train`, `validate`, or `generate` (default).
    let argv: Vec<String> = std::env::args().collect();
...
/// Rule-accuracy evaluation (the GPU counterpart of `eval_rule.py`) for weights
/// trained on the Rust corpus. Teacher-forced next-token accuracy vs the known
/// rule; the headline number is accuracy on a never-seen orbit at full context.
fn run_eval(flags: &[String]) {
    ...
    // teacher-forced next-token accuracy over windows of length `ctx`
    ...
    // New-orbit accuracy at several context lengths; headline = best length.
    // (A model trained at sequence length T only saw RoPE positions 0..T-1, so
    // evaluating beyond T is position-extrapolation, not a learning failure —
    // the peak is the fair, training-length-agnostic quality metric.)
    ...
    println!("  NEW   orbit       : {:5.1}%   <- overall rule-fit (mostly recall; ~92% of states are seen)", 100.0 * best_acc);
    let s: Vec<String> = sweep.iter().map(|(l, a)| format!("{l}:{:.0}%", 100.0 * a)).collect();
    println!("new-orbit acc by context length: {}", s.join("  "));
}

fn run_train(flags: &[String]) {
    let mut a = train::TrainArgs {
        ...
        wd: 0.1,
        seed: 0,
        // NOTE: distinct from the Python export's default (moe.weights) so a bare
        // `moe train` never silently clobbers a PyTorch-exported checkpoint.
...
fn run_generate() {
    let args = parse_args();
    // Debug: dump the last-position logits for the prompt (teacher-forced, no
    // generation) so they can be compared against PyTorch.
